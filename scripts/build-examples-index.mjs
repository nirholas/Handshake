...
const lines = [
	...
	'',
	`The repo ships ${payload.counts.total} examples: ${payload.counts.html_demos} web component demos, ` +
		`${payload.counts.projects} end-to-end projects, and ${payload.counts.packages} package example sets. ` +
		'This table is generated from what is on disk, so it cannot list one that was deleted or miss one that was added. ' +
		'The machine-readable version is [data/examples.json](../data/examples.json).',
	'',
];

for (const [group, items] of byGroup) {
	lines.push(`### ${group}`, '');
	lines.push('| Example | What it shows | Run it |', '|---|---|---|');
	for (const item of items) {
		const link = `[\`${item.path}\`](../${item.path})`;
		const what = mdEscape(item.description) || 'See the directory for details.';
		const run = item.run ? `\`${mdEscape(item.run)}\`` : 'Reference files, nothing to execute';
		lines.push(`| ${link} | ${what} | ${run} |`);
	}
	lines.push('');
}
...
if (startAt !== -1 && endAt !== -1 && endAt > startAt) {
	next = `${doc.slice(0, startAt)}${block}${doc.slice(endAt + END.length)}`;
} else if (startAt === -1 && endAt === -1) {
	next = `${doc.trimEnd()}\n\n---\n\n${block}\n`;
} else {
	throw new Error(
...
// A generator that can shrink a hand-written doc is a generator that will
// eventually eat one. The prose outside the markers must survive every run.
