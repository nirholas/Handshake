...
// docs/ALL.md is the generated concatenation of every other doc. Indexing it
// would duplicate the entire corpus and let one 6 MB file outrank the real page
// for every query.
const SKIP_FILES = new Set(['ALL.md']);

// Sections split on H1/H2 only. H3 splitting was measured at ~7.8k sections
// against ~3.4k here, and the smaller units lost the surrounding context that
// makes a result row readable while nearly doubling the artifact.
...
function walkMarkdown(dir, out = []) {
	for (const entry of readdirSync(dir, { withFileTypes: true }).sort((a, b) => (a.name < b.name ? -1 : 1))) {
		...
		} else if (entry.name.endsWith('.md') && !SKIP_FILES.has(relative(DOCS_DIR, full))) {
			out.push(full);
		}
	}
	return out;
