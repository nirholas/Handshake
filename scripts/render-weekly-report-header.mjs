...
//   node scripts/render-weekly-report-header.mjs <slug> [kicker] [title] [subtitle]
//   e.g. node scripts/render-weekly-report-header.mjs the-first-19-weeks three.ws "The First 19 Weeks"
//        node scripts/render-weekly-report-header.mjs weekly-report-2 "Weekly Report" "#2" "August 31 to September 6, 2026."
...
const slug = process.argv[2] || 'the-first-19-weeks';
const kicker = process.argv[3] || 'three.ws';
const title = process.argv[4] || 'The First 19 Weeks';
const subtitle = process.argv[5] || 'Everything we have shipped so far. April 14 to August 25, 2026.';
const outDir = resolve(root, `marketing/${slug}/images`);
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  h1 { position:relative; font-size:${Math.round(height*Math.min(0.19, 1.9/Math.max(title.length,4)))}px; font-weight:800; line-height:.95; letter-spacing:-.045em; background:linear-gradient(120deg,#fff 0%,#dcd6ff 45%,#ffd9c7 100%); -webkit-background-clip:text; background-clip:text; color:transparent; }
  ...
  <div class="body"><div><div class="kicker">${kicker}</div><h1>${title}</h1><p>${subtitle}</p></div>
