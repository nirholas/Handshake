...
//   node scripts/build-weekly-report-kit.mjs [issue=1]
...
const md = readFileSync(resolve(root, `docs/weekly-report-${issue}.md`), 'utf8');
const kitDir = resolve(root, `marketing/weekly-report-${issue}/kit`);
const imgDir = resolve(kitDir, 'images');
mkdirSync(imgDir, { recursive: true });

const sources = [resolve(root, `marketing/weekly-report-${issue}/images`), resolve(root, 'docs/media')];
const order = [];
let n = 0;
...
const html = `<!doctype html><html><head><meta charset="utf-8"><title>Weekly Report #${issue}</title>
...
writeFileSync(resolve(kitDir, `weekly-report-${issue}.html`), html);

const manifest = ['# Image insertion order', '', 'Numbers match the placeholders in weekly-report-' + issue + '.html. Cover image is separate (cover-x-article.png).', '']
...
for (const [from, to] of [
  [`marketing/weekly-report-${issue}/images/header-x-article.png`, 'cover-x-article.png'],
  [`marketing/weekly-report-${issue}/images/header-16x9.png`, 'announce-post-16x9.png'],
]) if (existsSync(resolve(root, from))) copyFileSync(resolve(root, from), resolve(kitDir, to));

execFileSync('zip', ['-qr', `weekly-report-${issue}-kit.zip`, '.'], { cwd: kitDir });
