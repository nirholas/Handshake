...
> MCP tools from [three.ws](https://three.ws), **five need no payment, wallet, or API key to start**: `forge_free` (text prompt → 3D GLB on the free NVIDIA NIM / Microsoft TRELLIS lane), `vanity_premium` (browse the pre-ground premium vanity inventory), and the crypto-news trio (`crypto_news`, `crypto_news_digest`, and `crypto_news_archive`, archive search is free up to a daily quota, then $0.001 USDC per search via x402). The rest are paid: text/image→3D mesh generation, 3D avatars, one-call text→rigged-avatar generation, GLB auto-rigging, conversational model refinement, material restyling, pose seeds, pump.fun snapshots, ERC-8004 agent reputation, ENS/SNS resolution, agent-to-agent delegation and end-to-end agent hiring, token sentiment, AgenC coordination reads, aixbt market intel, and a Solana vanity grinder. Paid calls are settled in USDC via the [x402](https://x402.org) payment protocol on Solana mainnet (`exact` scheme). No subscription, no API key, pay per call, and failed calls never bill the caller.
...
| `rig_mesh`       | $0.20  | Auto-rig a static GLB into an animation-ready model: humanoid skeleton + per-vertex skin weights via Make-It-Animatable. Takes a `glb_url` (e.g. `mesh_forge`'s output), returns the `riggedGlbUrl` and a three.ws pose-studio link.                                                                                                                                                                                                                      |
| `forge_avatar`   | $0.45  | **Text/image → rigged, animation-ready avatar in ONE call.** Chains the whole pipeline: Granite prompt director → FLUX + TRELLIS/Hunyuan3D reconstruction → Make-It-Animatable rigging, so the result loads straight into the pose studio and plays the canonical idle/walk library. Takes a `prompt`, a single `image_url`, or 1-4 `image_urls` (front/back/left/right) for higher-fidelity multi-view. A humanoid gate rejects a clearly non-humanoid subject **without charge** (use `mesh_forge` / `forge_free` for those, or set `allow_non_humanoid`). Returns the rigged GLB, the intermediate mesh, a pose-studio link, the directed prompt, and per-stage timing. |
...
| `refine_model`   | $0.25  | Iterate on a model by describing a change in plain language ("make it metallic", "bigger helmet"). Re-generates a **new** version anchored to the previous one (the prior prompt is folded with your change; an optional reference image anchors it as image→3D): real generation, never a fake diff. Each refinement appends to an immutable parent→child version lineage, so you can branch from an earlier version (`parent_index`) or revert. Pass `parent_lineage` to extend a thread. Returns the new GLB, a viewer link, the composed prompt, the lineage, and the active version index. |
| `restyle_material` | $0.05 | Re-skin an existing GLB **without regenerating its mesh** (geometry and UVs preserved exactly). Two modes: `instruction` ("make it chrome", "cyberpunk neon") for an AI-proposed PBR material, or `preset` (+ `seed`/`count`) for N reproducible colorway variants: same preset + seed always yields the same set. Shares `refine_model`'s lineage, so a restyle is just another version; the source GLB is never modified. Returns durable GLB URL(s), the applied PBR factors, and the lineage. |
...
| `sentiment_pulse` | $0.003 | Sentiment for a Solana token: scores the coin's recent pump.fun callouts (plus optional caller-supplied snippets) with the three.ws deterministic lexicon. Returns overall + per-source breakdown with examples. Pairs with `pump_snapshot`.                                                       |
...
| `vanity_premium`  | FREE   | Browse the **premium** vanity inventory: long (4-5+ char) brandable Solana addresses ground ahead of time on batch CPU and sold from stock, which the live grinder cannot reach. Read-only and free: returns each address, its rarity tier, a difficulty-scaled price ($1-$50), and a ready-to-pay x402 `buyUrl`. Paying that URL (USDC on Base or Solana) reveals the private key exactly once; the ciphertext is destroyed on delivery. Filter by prefix or tier, sort by rarity/price/newest. Keys are platform-generated: use as a token mint or sweep to self-generated custody, not as a treasury. |
...
| `crypto_news`     | FREE   | Live crypto headlines aggregated natively by three.ws from 197 publisher RSS/Atom feeds in 17 languages. Filter by category, source, or language, or full-text search. Each article carries the publisher link, publish time, detected tickers, and lexicon sentiment. No payment, no key.       |
...
| `agent_hire_discover`   | $0.01   | **Step one of the commerce loop.** Shortlist three.ws agents to hire for a task, ranked by task fit + live ERC-8004 on-chain reputation + real engagement. Each candidate carries its reputation evidence, capability/task-match score, and the exact USDC price `agent_hire` will settle. Optionally gate by a minimum reputation. |
| `agent_hire`            | $0.05   | **Step two.** Hire an agent end to end: quote the price up front, settle real USDC via x402, run the remote agent, and return its result **plus** a provenance receipt (agent, reputation, amount paid, on-chain settlement reference, latency) rendered as an inline MCP Apps receipt card. Enforces hard spend caps (per-call `MCP_AGENT_HIRE_MAX_PER_CALL_USD`, per-session `..._PER_SESSION_USD`), a `confirm` threshold, and an optional reputation floor: a blocked or failed hire never charges the caller. Needs `MCP_AGENT_TALK_TOKEN`. |
