...
- **Going Pro?** → [Paid plans](./plan-checkout.md): upgrade with a single on-chain payment in USDC, SOL, or $THREE (the platform coin takes 20% off); [hold-to-access](./hold-to-access.md) covers the separate hold-$THREE tier ladder; [the $THREE thesis](./three-thesis.md) is the long-form case for the token with every utility, sink, listing, and risk in one place; [Weekly Report #1](./weekly-report-1.md) is the week-by-week record of everything shipped since the first commit
