...
- **Want the agent to reach past the screen?** → [three.ws Home](./smart-home.md): the plan for an agent that runs your house, and the [`@three-ws/home-bridge`](../packages/home-bridge/README.md) package that already connects one to a real Home Assistant. Live state from your rooms so a 3D scene dims when your real lights dim, "good night" resolved to the scene you already built yourself, and a gate that stops and asks before anything unlocks a door. What that connection stores, for how long, and how to delete it: [Home privacy and retention](./home-privacy.md). Sharing a house is [Households](./home-households.md): five roles, per-member scopes, and the rule that a guest can turn a light on and can never authorise unlocking a door. What stands between a stranger and your deadbolt, written down and enforced by tests: [Home security](./home-security.md)
