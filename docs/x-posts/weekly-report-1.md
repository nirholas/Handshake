# X posts: Weekly Report #1

Copy to announce the first weekly report once it is published as an X Article. Posting is owner-gated; these are ready-to-paste drafts. The article body is `docs/weekly-report-1.md`; its cover is `marketing/weekly-report-1/images/header-x-article.png`. Attach [../../marketing/weekly-report-1/images/header-16x9.png](../../marketing/weekly-report-1/images/header-16x9.png) to the main post.

Every number below is verified in the article's "numbers first" section and its sources; do not add figures that are not in the article.

## Main post (@trythreews)

Attach: marketing/weekly-report-1/images/header-16x9.png, then the X Article link

```
Weekly Report #1 is out, and because it is the first one it covers everything: 19 weeks, week by week, from the first commit to today.

9,508 commits. 21 contributors. 2,674 changelog entries. 725 pages. 101 npm packages. 72 MCP servers. 4,519 x402 endpoints. 110,416 on-chain settlements.

One token: $THREE.

Read it here:
```

## Reply 1 (what is inside)

```
Inside:

Every week from April 14 to August 25: what shipped, what we announced, which partnerships landed
The full partnership list in one place: OpenAI, IBM, NVIDIA, Google Cloud, AWS, Alibaba Cloud, and more
Where the open source stems: 111 repos, 12 EVM chains, Hugging Face, Blender, ComfyUI, VS Code
Where $THREE sits in all of it
```

## Reply 2 (the honest part)

```
It also says what we have not done well: 410 shipped surfaces, 47 ever announced. Things like three-token-mcp, Forge Max, Motion Swap, Vaults, the Labor Market, and Coin Wars never got a post.

That is inventory, not a roadmap of promises. Weekly from now on.
```

## Reply 3 (the ask)

```
Build with it. Everything in the report is Apache-2.0 at github.com/nirholas/three.ws, and the world is open at three.ws/play.

$THREE: FeMbDoX7R1Psc4GEcvJdsbNbZA3bfztcyDCatJVJpump
```

## Telegram (@three_ws)

Attach: marketing/weekly-report-1/images/header-x-article.png, then the X Article link

```
Weekly Report #1 is out. The first one covers all 19 weeks since the first commit: 9,508 commits, 2,674 changelog entries, 725 pages, 101 npm packages, 72 MCP servers, 4,519 x402 endpoints, every partnership, and where $THREE sits in all of it. Read it here:
```
