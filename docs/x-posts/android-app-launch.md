...
The artwork and the X copy live in [`../../marketing/android-launch/`](../../marketing/android-launch/), generated by `npm run build:x-grid`. One composition is drawn at 4096x2304 and quartered, so X's 2x2 collage reassembles into the picture that was drawn: every quadrant of a 16:9 rectangle is itself 16:9, which is the aspect of the cell it lands in, so nothing is cropped.

**Attach the tiles in the order in [`kit/IMAGE-ORDER.md`](../../marketing/android-launch/kit/IMAGE-ORDER.md).** X fills the grid in attachment order, and any other order scrambles the composition. Per-image alt text and the follow-up replies are in [`kit/post.md`](../../marketing/android-launch/kit/post.md).
