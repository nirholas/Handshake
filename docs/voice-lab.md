...
- **Server config.** ElevenLabs must be configured (`ELEVENLABS_API_KEY`) or the endpoints return `503 not_configured`.
...
- **Synthesis limits.** 500 characters per playground request; 1000 characters per hour per user (reserved-then-refunded so a failed synth does not burn quota). Clips are cached in R2 for 30 days.
