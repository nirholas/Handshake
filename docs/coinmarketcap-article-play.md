...
And yet that community lives in the two saddest tabs on the internet: a Telegram group on one side, a price chart on the other. The thing everyone agrees is the whole point — the people — has no place. You can't walk into a coin. You can't see who else is there. You can't feel the market move.

[three.ws](https://three.ws) **/play** changes that. It turns every [pump.fun](https://pump.fun) coin into its own live 3D world — a shared, walk-around space where holders show up as real 3D avatars, talk to each other by voice, and watch the coin's on-chain activity ripple through the environment around them in real time. The coin's chart isn't on a screen somewhere else. The chart is the *weather*.

It's live now at **[three.ws/play](https://three.ws/play)**, in the browser, no install, no extension. Pick a coin and you're standing in it.
...
A crowd you can't talk to is just wallpaper. /play has **spatial voice chat**, and it's the thing that makes a coin's world feel like a real gathering instead of a chat box with 3D models.
...
A community's world isn't read-only. /play includes **collaborative voxel building**: a block palette and a build mode where anyone in the world can place and break blocks together, in real time, on a shared grid in the middle of the plaza.
...
4. Drop in. Walk around. Tap the mic to talk to whoever's there. Watch the world react as the trades land. Build something. Buy the coin.
