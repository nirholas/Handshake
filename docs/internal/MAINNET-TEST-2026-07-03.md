...
_Historical record: a point-in-time test report from 2026-07-03, when production still ran on Vercel. Production migrated to Google Cloud Run on 2026-07-07 (env vars now live on the Cloud Run service, not Vercel; deploy is `npm run deploy:gcp:full`, which runs `build:gcp` then `deploy:gcp`). The Vercel references, `vercel env` diagnostics, and secret-outage below describe that day's state. Do not read them as current infrastructure guidance. Current ops: [docs/ops/gcp-production.md](../ops/gcp-production.md)._

_Status as of 2026-08-05: the blocking outage below is resolved. The report's own verification step, `GET https://three.ws/api/auth/siws/nonce`, returns **200** with a real nonce (checked 2026-08-05), which per the corrected root cause means `JWT_SECRET` is restored in production; wallet login works again and the "test is BLOCKED at authentication" state no longer holds. Custodial key handling has since moved to a dedicated `WALLET_ENCRYPTION_KEY` with `JWT_SECRET` dual-read for v1 records ([api/_lib/secret-box.js](../../api/_lib/secret-box.js); see H8 in [docs/security/SECURITY_AUDIT.md](../security/SECURITY_AUDIT.md)). Since 2026-08-11 an unset `JWT_SECRET` only drops that legacy candidate from the decrypt list; it no longer breaks decryption of records written under the dedicated key, so the exact failure below (an empty `JWT_SECRET` locking every wallet) cannot recur on a deployment that holds `WALLET_ENCRYPTION_KEY`. The findings below are preserved as written._
