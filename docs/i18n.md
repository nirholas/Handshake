...
i18next). Ours lives inline in 570+ static HTML pages, so we add an **extraction**
...
real copy. It only touches **multi-word** phrases: a single word that matches
English is usually correct (`Avatar`, `Solana`, `3D`) and re-translating it risks
damaging a right answer. Locales are processed worst-first, and it refuses to
...
keys already done. For a catalog that is complete but trips the glossary/placeholder
lint, `npm run i18n:repair -- --locale=<code>` re-translates only the failing keys.
A locale that cannot be completed should be left out of `outputLocales` entirely
rather than committed partial, so the switcher never offers it.
