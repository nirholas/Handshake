...
Non-manual markers cover brows, eyes and mouth on avatars that carry ARKit blendshapes; the reference avatar on /sign-language has none, so its markers are compiled into the clip with nothing to drive. Marker coverage is also a fraction of what the face does in real ASL, which includes mouth morphemes, eye gaze, and body shift that this does not model.
