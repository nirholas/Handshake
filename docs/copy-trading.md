...
Both are Solana-native. Both run on `mainnet` and `devnet`. Neither is a paper-trading mode: there is no simulation layer anywhere in this stack, and no synthetic fills. Read [Simulated versus executed](#simulated-versus-executed) before you enable anything.
