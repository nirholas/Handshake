...
- **Decision points / branches:** signed-in vs redirect to login; owned avatars vs the default-avatar fallback; public (CDN URL) vs private (signed-URL resolve) avatar; presign success vs presign failure (413 too-large / 429 rate-limit / generic, each with its own message; no data-URI fallback exists); prompt vs no prompt; done vs failed vs 20-minute timeout; `402 free_trial_used` retargets the retry button to "Upgrade plan" pointing at `/dashboard`.
