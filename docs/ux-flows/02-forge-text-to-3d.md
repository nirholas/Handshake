...
- `/forge` → `pages/forge.html` (loads `src/forge.js` + 19 sibling `forge-*` modules).
...
- **Source:**
  - `pages/forge.html` (4153 lines: full composer/stage markup + styles)
  - `src/forge.js` (3213 lines: main controller, catalog, engine/tier selection, job submit, polling, result, gating; imports `src/forge-timeline.js` for the generation stage timeline and `src/forge-compare.js` for side-by-side compare)
...
- **Steps (core text→3D path):**
  ...
  19. (optional) **Rig for animation** (`forge-rig.js`): adds a humanoid skeleton + skin weights to the current GLB via `POST /api/forge?action=rig` (self-hosted GCP UniRig, Replicate fallback), polled through the same `GET /api/forge?job=` contract.
  20. (optional) **Optimize topology** (`forge-optimize.js`): tri/quad/low-poly remesh via `POST /api/forge-remesh` (polled).
  21. (optional) **Game-Ready** (`forge-gameready.js`): retopologize to a poly budget and export a textured GLB + FBX for Unity/Unreal via `POST /api/forge-gameready` (polled), with budget slider + wireframe preview.
...
- **External calls / dependencies:**
  ...
  - Galleries: `GET /api/forge-gallery?limit=24` (your creations), `GET /api/forge-gallery?scope=community&limit=24` (showcase, plus `&sort=` for the Forge-Off board), `POST /api/forge-vote` (showcase upvotes), `GET /api/forge-creation?id=` (share open)
  ...
  - **Photoreal reference-image path (the realism lever):** the final mesh reads as real only if the reference image the reconstructor sees is a photoreal, single-subject, plain-seamless-background, evenly-lit photograph. Two pieces drive that:
    ...
    - `api/_lib/forge-reference-image.js` → `generateReferenceImage(directedPrompt, { aspectRatio, negativePrompt?, seed, skipNim })`: generates the reconstruction reference image on the Vertex Gemini 2.5 Flash Image lane (`imageConfig.imageSize:"2K"`), tuned as a plain-background studio photo, with the director's negatives folded in as natural-language avoidance. Automatic fallthrough to the existing provider chain (`textToImage`: NIM FLUX free → Vertex Imagen → Replicate), returning the same `{ imageUrl, model }` shape. When `negativePrompt` is omitted it derives subject-aware negatives from the prompt itself, so a caller gets them for free. `api/forge.js` now imports it on the text→3D path (dynamic import with `textToImage` as the fallback).
