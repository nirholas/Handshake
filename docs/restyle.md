...
- **Preset library (14 looks).** One click applies a physically based material across every standard material in the model. The presets are `chrome`, `gold`, `copper`, `brushedSteel`, `gunmetal`, `matte` (plastic), `glossy` (plastic), `rubber`, `ceramic`, `glass`, `wood`, `stone`, `neon`, and `holographic`. Each carries tuned color, metalness, roughness, environment-map intensity, and (for neon and holographic) emissive values.
