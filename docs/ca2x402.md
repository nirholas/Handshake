...
2. **Resolve.** `GET /api/ca2x402/resolve` pulls the token's live market from
   DexScreener and returns both the token preview and the generated service
   descriptor. This read is free and unauthenticated.
...
2. **Fetch the market.** `fetchTokenMarket()` calls the public DexScreener token
   endpoint (no key) with a 6 second timeout, then picks the pair with the
   deepest USD liquidity across every chain and DEX DexScreener knows for that
   address. The address may sit on either side of the pair; the reader selects
   whichever side matches what you passed.
3. **No market found.** A well-formed address with no live pair (unlaunched, dead
   liquidity, wrong chain, or an upstream hiccup) returns
   ...
   the gap.
4. **Success.** `200 { ok: true, token, service }` with
...
| `endpoint`                       | `<origin>/api/x402/token-intel?mint=<MINT_ADDRESS>`                          |
| `method`                         | `GET`                                                                        |
...
The endpoint origin is derived from the request (`x-forwarded-proto` /
`x-forwarded-host`, falling back to `three.ws`), so a local dev resolve generates
a local endpoint and production generates a production one.
...
[x402 revenue and receipts](x402-revenue.md)).
...
- `503 data_unavailable` when DexScreener has no live market or no 24 hour change
  for the token.
...
**cURL.** Step one shows the price by taking the `402`; step two pays and calls in
one command:
...
npx x402-fetch "https://three.ws/api/x402/token-intel?mint=<MINT_ADDRESS>"
```
...
- [x402 revenue and receipts](x402-revenue.md): where a settled payment for this
  endpoint is recorded and how the buyer's receipt is issued.
