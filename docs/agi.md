...
2. Read **The mind, out loud**: a live stream of the agent's decisions. Each shows the kind of call, the coin (linked to Solscan), the rationale, the confidence it stated at the time, and, once the trade resolves, whether it was right and the realized P&L, with a proof link to the exit.
...
- **Graceful degradation.** Each truth layer degrades to null on failure rather than failing the page. An explicit `?agent=` that resolves to a private or missing agent is a `404`; the auto and env paths only ever select public agents.
