...
Consumed by the hosted embed [pages/embodiment/embed.html](../../pages/embodiment/embed.html) (`/embodiment/embed`, framable by ChatGPT, Claude, and any other host). The reply-to-rig core (lip-sync lanes, emotion classifier, rig-mode gate) lives in [src/embodiment/](../../src/embodiment/); each module there carries a header comment describing what it owns, and the behavior is pinned by `tests/embodiment-*.test.js`.
