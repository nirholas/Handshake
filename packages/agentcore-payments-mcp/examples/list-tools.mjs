// list-tools.mjs: print every tool @three-ws/agentcore-payments-mcp advertises,
// with its input schema and its MCP annotation hints.
//
// Spawns this package's own server over stdio the way the README runs it
// (`node src/index.js`), performs the MCP initialize handshake, calls
// tools/list, and prints each tool.
//
//   node examples/list-tools.mjs
//
// No credentials and no network: registration is env-free, so the server
// advertises all five tools with no THREE_WS_SESSION or PAYMENT_SESSION_TOKEN
// set. Nothing here can spend money.
//
// Read the hints column. `pay_with_session` is the only non-read-only tool that
// moves funds, and it is the one an MCP client should surface a confirmation for.
...
// { command: 'npx', args: ['-y', '@three-ws/agentcore-payments-mcp'] } to
// exercise the published package instead of this checkout.
 ...
 *
 * A tool whose four hints are all `false` is a deliberate declaration (a
 * non-destructive, non-idempotent, closed-world write), not a missing one, so
 * it must not read the same as a tool that was never annotated. An MCP client
 * decides whether to prompt the user off exactly this distinction.
 */
function hints(annotations) {
	if (!annotations) return 'UNANNOTATED (client must assume the worst)';
	const flags = [];
	...
	return 'write, non-destructive, non-idempotent, closed-world';
}
...
const client = new Client({
	name: 'agentcore-payments-mcp-list-tools-example',
	version: '1.0.0',
});
...
const spending = tools.filter((t) => t.annotations && t.annotations.readOnlyHint !== true);
console.log(`\nTools that change state: ${spending.map((t) => t.name).join(', ') || 'none'}`);
console.log('Only pay_with_session moves funds, and only up to the session budget you set.');
