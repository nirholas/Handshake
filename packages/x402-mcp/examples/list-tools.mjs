// list-tools.mjs: print every tool @three-ws/x402-mcp advertises, with its
// input schema.
//
...
//
// No key, no signer, no network payment: registration is env-free, so the
// server advertises all four tools even with no SOLANA_SECRET_KEY set.
...
// { command: 'npx', args: ['-y', '@three-ws/x402-mcp'] } to exercise the
...
const client = new Client({ name: 'x402-mcp-list-tools-example', version: '1.0.0' });
