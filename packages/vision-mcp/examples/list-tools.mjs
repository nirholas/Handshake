// list-tools.mjs: print every tool @three-ws/vision-mcp advertises, with its
// input schema.
//
...
//
// No key and no image: registration is env-free, so the server advertises all
// three tools with nothing configured.
...
// { command: 'npx', args: ['-y', '@three-ws/vision-mcp'] } to exercise the
...
const client = new Client({ name: 'vision-mcp-list-tools-example', version: '1.0.0' });
