...
import { GeneralChatAgent } from "../src/agents/GeneralChatAgent.js";
describe("GeneralChatAgent", () => {
  ...
  const createMockContext = (phase, payload) => ({
    phase,
    payload,
    session: {
  ...
  describe("tool_result phase", () => {
    describe("GTD async tasks", () => {
        ...
        const context = createMockContext("tool_result", {
          ...
          data: {
            state: {
              ...
              task: { instruction: "Do something async", timeout: 3e4 }
            }
          }
        });
        const result = await agent.runner(context, state);
      ...
      it("should return exec_tasks for multiple async tasks (execTasks)", async () => {
        ...
        const state = createMockState();
        const tasks = [
          { instruction: "Task 1", timeout: 3e4 },
          { instruction: "Task 2", timeout: 3e4 }
        ];
        const context = createMockContext("tool_result", {
          parentMessageId: "tool-msg-1",
          stop: true,
      ...
      it("should not trigger exec_task when stop is false", async () => {
        ...
        const context = createMockContext("tool_result", {
          parentMessageId: "tool-msg-1",
          stop: false,
          // stop is false, should not trigger exec_task
          data: {
            state: {
              type: "execTask",
              parentMessageId: "exec-parent-msg",
  ...
  describe("unknown phase", () => {
    it("should return finish instruction for unknown phase", async () => {
      ...
      const state = createMockState();
      const context = createMockContext("unknown_phase");
      const result = await agent.runner(context, state);
      expect(result).toEqual({
        type: "finish",
      ...
      const state = createMockState({
        toolManifestMap: {
          bash: {
            identifier: "bash",
            humanIntervention: "never"
            // Tool doesn't require approval by default
          }
        },
        userInterventionConfig: {
          approvalMode: "allow-list",
          allowList: ["bash/bash"]
          // Only bash/bash is allowed
        }
      });
      const context = createMockContext("llm_result", {
        hasToolsCalling: true,
  ...
  describe("headless mode (for async tasks)", () => {
    ...
    it('should execute tools with "always" policy in headless mode', async () => {
      ...
      const state = createMockState({
            ...
            api: [
              {
                name: "installPlugin",
                humanIntervention: "always"
                // Always requires intervention normally
              }
            ]
          }
        },
        userInterventionConfig: {
          approvalMode: "headless"
          // Headless mode bypasses even 'always'
        }
      });
      const context = createMockContext("llm_result", {
        hasToolsCalling: true,
    ...
    it("should skip security blacklisted tools in headless mode", async () => {
      ...
      const blacklistedTool = {
        id: "call-1",
        identifier: "bash",
        apiName: "bash",
        arguments: '{"command":"rm -rf /"}',
        // Matches security blacklist
        type: "builtin"
      };
      const state = createMockState({
        toolManifestMap: {
