...
| `POST /api/notifications/:id/read` | Mark a single notification as read | Idempotent (`read_at = coalesce(read_at, now())`); returns `{ id, read_at }`, 404 if the row is not the caller's. Handler: [`[id]/read.js`](./%5Bid%5D/read.js) |
...
| `PUT /api/notifications/preferences` | Save preference overrides | Body `{ categories: { <cat>: { <channel>: bool } }, telegram_chat_id? }`; sanitised sparse overlay, unknown keys dropped, returns the re-resolved matrix |
