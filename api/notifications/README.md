...
| `POST /api/notifications/track` | Record a re-engagement funnel event | Body `{ notification_id?, channel: 'push'\|'in_app'\|'avatar', event: 'opened'\|'returned' }`. Closes the sent-to-opened-to-returned loop: the service worker ([`public/push-sw.js`](../../public/push-sw.js)) fires `opened` on push click, the corner companion ([`../../src/notification-herald.js`](../../src/notification-herald.js)) fires `opened` on a click-through from a line it announced, and the app fires `returned` when it boots from `?source=push`. Session-auth only, CSRF-exempt idempotent beacon (deduped by a partial unique index); `sent` rows are written server-side by [`../_lib/notify.js`](../_lib/notify.js) |
| `GET /api/notifications/preferences` | Read the full resolved preference matrix | Returns `{ categories, channels, type_categories, prefs, push: { subscribed_devices } }`, everything the settings UI renders from. `type_categories` is the server's type-to-category map, shipped so browser clients (the avatar channel) resolve a category without a request per notification |
...
- `CHANNELS`: `in_app` (the bell inbox, on by default for every category), `push` (Web Push, on by default but only reaches users who subscribed a device), `email` (defaults on only for money and security categories), `telegram` (opt-in, needs a linked chat id), `avatar` (the corner companion walks on screen and delivers the line, defaults on for money, finished creations, and account events; delivered client-side by [`../../src/notification-herald.js`](../../src/notification-herald.js), which owns the freshness, dedupe and batch rules).
