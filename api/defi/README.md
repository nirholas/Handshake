...
- **Caching and dedup.** In-memory caches (5 to 10 min per feed, bounded maps for per-slug/per-id lookups), in-flight request dedup on the heavy yields fetch, and CDN `s-maxage` with `stale-while-revalidate` keep upstream load at a handful of requests per instance per TTL regardless of traffic.
