/**
 * GPT Forge: the ChatGPT-dedicated forge pipeline, an exact clone of
 * api/forge.js serving the ChatGPT surfaces (the Apps SDK MCP connector at
 * api/mcp-studio and the custom-GPT Actions endpoint at api/3d/studio, both
 * via api/_mcp-studio/gpt-forge-client.js). It exists so the ChatGPT pipeline
 * can be improved and tuned independently without touching /api/forge or any
 * surface that rides it. Owner directive 2026-07-18: keep /api/forge frozen;
 * evolve the ChatGPT path here. Jobs share the same forge_creations table,
 * job-token codec, lanes, and workers, so a job submitted here can be polled
 * on either endpoint.
 *
 *   POST /api/gpt-forge   { prompt, aspect_ratio?, path?, tier?, backend? }  → text→3D
 *   POST /api/gpt-forge   { image_urls[], prompt?, path?, tier?, backend? }  → image→3D
 *   POST /api/gpt-forge   { image_urls[sketch], prompt, path: 'sketch' }     → sketch→3D
 *   POST /api/gpt-forge?action=rig  { glb_url }                              → auto-rig
 *   GET  /api/gpt-forge?job=<id>                                             → poll a job
 *   GET  /api/gpt-forge?catalog                                              → tier/backend/cost matrix
 *
