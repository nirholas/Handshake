...
export const RING_CATALOG = [
	...
	},
	{
		...
		autobuy: false,
		weight: 0,
		businessEffect: 'Each settled call executes one small, real USDC→$THREE market buy on Jupiter (buy-only) funded by the micro-buy wallet, then the tokens are swept to the treasury.',
		note: 'autobuy:false — every settled call spends real USDC on a live $THREE market buy (bounded by THREE_MICROBUY_DAILY_CAP_USD). It has its own dedicated driver, api/cron/three-buy-loop.js, so the generic ring rotation must not also fire it.',
	},
	{
		slug: 'pump-launch',
