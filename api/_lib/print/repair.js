...
export function fillBoundaryLoops(positions, indices) {
	...
	if (!loops.length) return { positions, indices, filled: 0, addedTriangles: 0 };
...
export function cleanTopology(mesh, opts = {}) {
	...
	const cleaned = dropDegenerateTriangles(welded.positions, welded.indices, {
		minArea: (tolerance * tolerance) / 2,
	});
	const oriented = orientConsistently(cleaned.indices);
	const before = edgeCensus(oriented.indices);
	const filled = fillBoundaryLoops(welded.positions, oriented.indices);
	const after = edgeCensus(filled.indices);
	...
	return {
		...
		metrics: {
			...
			openEdgesBefore: before.open,
			openEdgesAfter: after.open,
			nonManifoldEdgesAfter: after.nonManifold,
			signedVolume: volume,
		},
	};
}
...
export async function repairMesh(mesh, opts = {}) {
	...
	const components = connectedComponents(arrays.indices);
	return {
		wasm,
		solid,
