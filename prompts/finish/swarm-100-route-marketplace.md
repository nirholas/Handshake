# Route audit: /marketplace (Marketplace)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/marketplace')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/marketplace" in vercel.json (the live
...
Route: /marketplace ("Marketplace", section "main").
Declared purpose: Buy access to agents, skills, and avatars from other creators.
...
1. Load http://localhost:3000/marketplace. Record the HTTP status, every console
...
- [ ] Local load of /marketplace: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-marketplace.md
