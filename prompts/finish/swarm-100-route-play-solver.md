# Route audit: /play/solver (The economy solver)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/play/solver')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/play/solver" in vercel.json (the live
...
1. Load http://localhost:3000/play/solver. Record the HTTP status, every console
...
- [ ] Local load of /play/solver: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-play-solver.md
