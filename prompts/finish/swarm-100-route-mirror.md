# Route audit: /mirror (Copy Trading)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/mirror')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/mirror" in vercel.json (the live
...
Route: /mirror ("Copy Trading", section "crypto").
Declared purpose: Follow a high-performing agent and your own agent mirrors its real trades, proportionally and strictly within your spend policy.
...
1. Load http://localhost:3000/mirror. Record the HTTP status, every console
...
- [ ] Local load of /mirror: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-mirror.md
