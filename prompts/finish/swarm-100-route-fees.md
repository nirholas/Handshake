# Route audit: /fees (Protocol Fees & Revenue)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/fees')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/fees" in vercel.json (the live
...
Route: /fees ("Protocol Fees & Revenue", section "crypto").
Declared purpose: Fees paid by users and revenue kept by protocols across all of DeFi, ranked with 24h/7d/30d totals and an aggregate history chart. Toggle between fees and revenue; each protocol links to its detail page. Live from DeFiLlama.
...
1. Load http://localhost:3000/fees. Record the HTTP status, every console
...
- [ ] Local load of /fees: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-fees.md
