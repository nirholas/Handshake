# Route audit: /objects (Object Library)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/objects')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/objects" in vercel.json (the live
...
Route: /objects ("Object Library", section "main").
Declared purpose: Hundreds of free CC0 3D objects and props: furniture, lighting, electronics, decor. Public domain, no attribution required. Preview live, place it in your room with AR, or download the GLB.
...
1. Load http://localhost:3000/objects. Record the HTTP status, every console
...
- [ ] Local load of /objects: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-objects.md
