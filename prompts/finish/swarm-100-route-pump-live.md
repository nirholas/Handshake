# Route audit: /pump-live (Pump Live)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/pump-live')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/pump-live" in vercel.json (the live
...
Route: /pump-live ("Pump Live", section "crypto").
Declared purpose: Real-time feed of every pump.fun launch as it happens, with a 3D agent that reacts live.
...
1. Load http://localhost:3000/pump-live. Record the HTTP status, every console
...
- [ ] Local load of /pump-live: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-pump-live.md
