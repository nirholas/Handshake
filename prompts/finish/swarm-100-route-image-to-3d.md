# Route audit: /image-to-3d (Image to 3D: Turn a Photo into a 3D Model)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/image-to-3d')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/image-to-3d" in vercel.json (the live
...
Route: /image-to-3d ("Image to 3D: Turn a Photo into a 3D Model", section "build").
Declared purpose: Upload a photo (or up to 4 angles) and get a downloadable textured 3D model (GLB). The Forge's photo lane, opened directly: for objects and props; for faces, use Selfie to Avatar.
...
1. Load http://localhost:3000/image-to-3d. Record the HTTP status, every console
...
- [ ] Local load of /image-to-3d: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-image-to-3d.md
