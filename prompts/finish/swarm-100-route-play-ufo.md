# Route audit: /play/ufo (Flappin UFO: retired demo)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/play/ufo')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/play/ufo" in vercel.json (the live
...
Route: /play/ufo ("Flappin UFO: retired demo", section "crypto").
Declared purpose: The Flappin UFO arcade demo ran on an external deployment that is no longer online, so the game has been retired. The page now points visitors to the live three.ws experiences instead.
...
1. Load http://localhost:3000/play/ufo. Record the HTTP status, every console
...
- [ ] Local load of /play/ufo: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-play-ufo.md
