# Route audit: /launches (Agent Launches)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/launches')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/launches" in vercel.json (the live
...
Route: /launches ("Agent Launches", section "crypto").
Declared purpose: Live public feed of every coin launched by a three.ws agent: market caps, graduation status, and the agent behind each launch.
...
1. Load http://localhost:3000/launches. Record the HTTP status, every console
...
- [ ] Local load of /launches: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-launches.md
