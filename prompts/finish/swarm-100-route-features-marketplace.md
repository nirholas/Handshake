# Route audit: /features/marketplace (Marketplace: Discover and Fork AI Agents)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/features/marketplace')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/features/marketplace" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/features/marketplace
...
Route: /features/marketplace ("Marketplace: Discover and Fork AI Agents", section "build").
Declared purpose: Browse hundreds of community-built AI agents with 3D avatars and on-chain identities. Fork any agent, buy paid skills, and ship in minutes.
...
1. Load http://localhost:3000/features/marketplace. Record the HTTP status, every console
...
- [ ] Local load of /features/marketplace: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-features-marketplace.md
