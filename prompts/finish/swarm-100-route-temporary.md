# Route audit: /temporary (Drive Your Avatar)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/temporary')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/temporary" in vercel.json (the live
...
Route: /temporary ("Drive Your Avatar", section "main").
Declared purpose: Drive a 3D avatar around with a joystick or WASD, then toggle the back camera for AR-style passthrough on your phone.
...
1. Load http://localhost:3000/temporary. Record the HTTP status, every console
...
- [ ] Local load of /temporary: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-temporary.md
