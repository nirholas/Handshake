# Route audit: /import/rpm (Import an avatar URL)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/import/rpm')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/import/rpm" in vercel.json (the live
...
Route: /import/rpm ("Import an avatar URL", section "build").
Declared purpose: Import any GLB or glTF avatar into three.ws and give it an agent brain.
...
1. Load http://localhost:3000/import/rpm. Record the HTTP status, every console
...
- [ ] Local load of /import/rpm: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-import-rpm.md
