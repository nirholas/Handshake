...
`/workspaces/three.ws`, or say "execute `prompts/finish/event-08-event-closeout.md`". Read
[00-CONTEXT.md](event-00-CONTEXT.md) and `CLAUDE.md` first.
...
1. Finish 100%. Never end with a question or an unexecuted plan. Delete this file when the
   definition of done is verified and log the outcome in [PROGRESS.md](event-PROGRESS.md); any
...
(`2bae5d8c2` or later, per the preflight entry in [PROGRESS.md](event-PROGRESS.md)) was serving
...
2. **Write the closeout entry** in [PROGRESS.md](event-PROGRESS.md): which world, the revision
...
4. **Retire the pack.** Verify [02-play-polish-sweep.md](event-02-play-polish-sweep.md) and
   [06-photo-mode-share.md](event-06-photo-mode-share.md) against their deliverables (the
