# Route audit: /markets/news/article (Crypto News Reader)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/markets/news/article')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/markets/news/article" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/markets/news/article
...
Route: /markets/news/article ("Crypto News Reader", section "crypto").
Declared purpose: Rich article reader for crypto news: full story extraction with AI summary, key points, sentiment, detected tickers, and related coverage from across the industry.
...
1. Load http://localhost:3000/markets/news/article. Record the HTTP status, every console
...
- [ ] Local load of /markets/news/article: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-markets-news-article.md
