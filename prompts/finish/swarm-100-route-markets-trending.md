# Route audit: /markets/trending (Trending Crypto)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/markets/trending')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/markets/trending" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/markets/trending
...
Route: /markets/trending ("Trending Crypto", section "crypto").
Declared purpose: The most-searched coins, categories, and NFTs on CoinGecko over the last 24 hours: trending rank, price, 24h change, market cap, and volume, each coin and category linking to its detail page. Auto-refreshing.
...
1. Load http://localhost:3000/markets/trending. Record the HTTP status, every console
...
- [ ] Local load of /markets/trending: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-markets-trending.md
