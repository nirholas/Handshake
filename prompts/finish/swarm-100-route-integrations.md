# Route audit: /integrations (Integrations)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/integrations')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/integrations" in vercel.json (the live
...
Route: /integrations ("Integrations", section "build").
Declared purpose: Drop-in 3D agents, chat widgets, walk companions, and live token embeds for any site. One script tag. Powered by three.ws.
...
1. Load http://localhost:3000/integrations. Record the HTTP status, every console
...
- [ ] Local load of /integrations: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-integrations.md
