# Route audit: /render-lab (Render Lab)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/render-lab')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/render-lab" in vercel.json (the live
...
Route: /render-lab ("Render Lab", section "build").
Declared purpose: Compose a real avatar render in the browser and copy the exact API call. Reads each model first (ARKit morph targets, canonical-skeleton coverage, geometry) and only offers controls that model will actually honor, so nothing you build here silently fails in production. Contact-sheet mode renders every framing, pose, or expression at once.
...
1. Load http://localhost:3000/render-lab. Record the HTTP status, every console
...
- [ ] Local load of /render-lab: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-render-lab.md
