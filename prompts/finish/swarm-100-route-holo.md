...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/holo')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/holo" in vercel.json (the live
...
Route: /holo ("Holo Sticker", section "labs").
Declared purpose: A live holographic foil sticker rendered in your browser: tilt it with your pointer, peel the corner, switch foils, die-cut your own text, and download the frame as a PNG. Fully procedural, no assets.
...
1. Load http://localhost:3000/holo. Record the HTTP status, every console
...
- [ ] Local load of /holo: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-holo.md
