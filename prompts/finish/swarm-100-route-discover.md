...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/discover')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/discover" in vercel.json (the live
...
Route: /discover ("Discover", section "main").
Declared purpose: On-chain agent directory (ERC-8004 + Solana). Browse, search, and inspect every registered agent.
...
1. Load http://localhost:3000/discover. Record the HTTP status, every console
...
- [ ] Local load of /discover: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-discover.md
