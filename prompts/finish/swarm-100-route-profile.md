# Route audit: /profile (Your creator portfolio)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/profile')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/profile" in vercel.json (the live
...
Route: /profile ("Your creator portfolio", section "account").
Declared purpose: Your public three.ws portfolio: every avatar, agent, forged 3D model, world, coin, skill, and plugin you've made, in one place. Signed out, this redirects to sign-in; every user's own page also lives at /u/:username.
...
1. Load http://localhost:3000/profile. Record the HTTP status, every console
...
- [ ] Local load of /profile: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-profile.md
