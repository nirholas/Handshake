# Route audit: /symphony (Agent Symphony)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/symphony')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/symphony" in vercel.json (the live
...
Route: /symphony ("Agent Symphony", section "labs").
Declared purpose: The agent economy, played live as generative music. Every real payment, trade, launch and level-up on three.ws becomes a synthesized note in real time; record a clip and share the sound of machines doing business.
...
1. Load http://localhost:3000/symphony. Record the HTTP status, every console
...
- [ ] Local load of /symphony: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-symphony.md
