# Route audit: /forever (Forever: etch a message into Bitcoin)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/forever')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/forever" in vercel.json (the live
...
Route: /forever ("Forever: etch a message into Bitcoin", section "crypto").
Declared purpose: Inscribe a message onto the Bitcoin blockchain. It stays there. Forever.
...
1. Load http://localhost:3000/forever. Record the HTTP status, every console
...
- [ ] Local load of /forever: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-forever.md
