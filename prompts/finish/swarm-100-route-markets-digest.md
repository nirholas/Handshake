# Route audit: /markets/digest (Crypto News Digest)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/markets/digest')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/markets/digest" in vercel.json (the live
...
Route: /markets/digest ("Crypto News Digest", section "crypto").
Declared purpose: The day's crypto news grouped into the handful of stories that actually matter: each with a plain-language summary, a market stance, the tickers involved, and every outlet that covered it. Clustered from 38 live publisher feeds.
...
1. Load http://localhost:3000/markets/digest. Record the HTTP status, every console
...
- [ ] Local load of /markets/digest: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-markets-digest.md
