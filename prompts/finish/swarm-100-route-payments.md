# Route audit: /payments (Agent Payment Sessions)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/payments')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/payments" in vercel.json (the live
...
Route: /payments ("Agent Payment Sessions", section "account").
Declared purpose: Create budget-limited payment sessions for AI agents. Governance-enforced spend limits, URL allowlists, and expiry: agents pay x402 APIs without holding a private key.
...
1. Load http://localhost:3000/payments. Record the HTTP status, every console
...
- [ ] Local load of /payments: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-payments.md
