# Route audit: /features/forge (Forge: Text & Image to 3D Models)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/features/forge')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/features/forge" in vercel.json (the live
...
Route: /features/forge ("Forge: Text & Image to 3D Models", section "build").
Declared purpose: Type a description or upload photos, and get a downloadable textured 3D model (GLB). Multiple self-hosted engines, from fast drafts to PBR realism.
...
1. Load http://localhost:3000/features/forge. Record the HTTP status, every console
...
- [ ] Local load of /features/forge: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-features-forge.md
