# Route audit: /features/play (Play: Live 3D Coin Worlds)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/features/play')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/features/play" in vercel.json (the live
...
Route: /features/play ("Play: Live 3D Coin Worlds", section "build").
Declared purpose: Every pump.fun coin gets a deterministic 3D world. Walk in as your avatar, chat with holders, and trade: all in the browser.
...
1. Load http://localhost:3000/features/play. Record the HTTP status, every console
...
- [ ] Local load of /features/play: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-features-play.md
