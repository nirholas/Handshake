# Route audit: /features/walk (Walk: 3D Avatar AR)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/features/walk')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/features/walk" in vercel.json (the live
...
Route: /features/walk ("Walk: 3D Avatar AR", section "build").
Declared purpose: Drive your AI agent's 3D avatar with WASD or joystick. Toggle AR mode and it walks on your real floor through your phone camera.
...
1. Load http://localhost:3000/features/walk. Record the HTTP status, every console
...
- [ ] Local load of /features/walk: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-features-walk.md
