# Route audit: /sniper/experiments (Sniper Experiments: rules vs LLM judgment, scored live)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/sniper/experiments')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/sniper/experiments" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/sniper/experiments
...
Route: /sniper/experiments ("Sniper Experiments: rules vs LLM judgment, scored live", section "crypto").
Declared purpose: The sniper fleet runs different rule sets side by side, including LLM-judged agents with no rules at all. The live scoreboard of which entry conditions actually make money, every fill verifiable on-chain.
...
1. Load http://localhost:3000/sniper/experiments. Record the HTTP status, every console
...
- [ ] Local load of /sniper/experiments: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-sniper-experiments.md
