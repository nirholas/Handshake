# Route audit: /scene (Scene Studio)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/scene')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/scene" in vercel.json (the live
...
Route: /scene ("Scene Studio", section "build").
Declared purpose: Full 3D scene editor in the browser: import GLB models, arrange them with transform gizmos, edit materials and lights, and export complete scenes.
...
1. Load http://localhost:3000/scene. Record the HTTP status, every console
...
- [ ] Local load of /scene: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-scene.md
