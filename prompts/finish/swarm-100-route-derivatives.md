# Route audit: /derivatives (Crypto Derivatives & Perpetual Futures)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/derivatives')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/derivatives" in vercel.json (the live
...
Route: /derivatives ("Crypto Derivatives & Perpetual Futures", section "crypto").
Declared purpose: Live perpetual-futures markets across exchanges: price, 24h change, funding rate, open interest, and volume for every major contract. Filter by underlying index (BTC, ETH, SOL) and sort by any column.
...
1. Load http://localhost:3000/derivatives. Record the HTTP status, every console
...
- [ ] Local load of /derivatives: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-derivatives.md
