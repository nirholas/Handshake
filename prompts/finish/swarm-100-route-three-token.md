# Route audit: /three-token ($THREE Token)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/three-token')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/three-token" in vercel.json (the live
...
Route: /three-token ("$THREE Token", section "crypto").
Declared purpose: The $THREE protocol token: live price, bonding-curve chart, streaming trades, and one-click buy.
...
1. Load http://localhost:3000/three-token. Record the HTTP status, every console
...
- [ ] Local load of /three-token: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-three-token.md
