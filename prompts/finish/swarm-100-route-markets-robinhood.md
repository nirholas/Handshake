# Route audit: /markets/robinhood (Robinhood Chain)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/markets/robinhood')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/markets/robinhood" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/markets/robinhood
...
Route: /markets/robinhood ("Robinhood Chain", section "crypto").
Declared purpose: The 24/7 tokenized-equity board for Robinhood Chain (4663): live Chainlink NAV vs DEX premium for every Stock Token, a memecoin screener across NOXA and The Odyssey launchpads, recent launches, and chain TVL: plus a real wallet-connect buy flow for its coins.
...
1. Load http://localhost:3000/markets/robinhood. Record the HTTP status, every console
...
- [ ] Local load of /markets/robinhood: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-markets-robinhood.md
