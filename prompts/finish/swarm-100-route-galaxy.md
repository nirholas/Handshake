# Route audit: /galaxy (Agent Galaxy)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/galaxy')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/galaxy" in vercel.json (the live
...
Route: /galaxy ("Agent Galaxy", section "labs").
Declared purpose: Every public agent embedded by IBM Granite on watsonx.ai into a navigable 3D constellation with natural-language semantic search.
...
1. Load http://localhost:3000/galaxy. Record the HTTP status, every console
...
- [ ] Local load of /galaxy: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-galaxy.md
