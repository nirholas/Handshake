# Route audit: /gas (Ethereum Gas Tracker)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/gas')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/gas" in vercel.json (the live
...
Route: /gas ("Ethereum Gas Tracker", section "crypto").
Declared purpose: Live Ethereum gas fees straight from the chain: slow, standard, and fast tiers in gwei with USD cost estimates for transfers, token sends, DEX swaps, and NFT mints. Real on-chain fee history, refreshed automatically, no API key required.
...
1. Load http://localhost:3000/gas. Record the HTTP status, every console
...
- [ ] Local load of /gas: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-gas.md
