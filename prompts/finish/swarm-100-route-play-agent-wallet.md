# Route audit: /play/agent-wallet (Agent Wallet (x402 on Solana))
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/play/agent-wallet')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/play/agent-wallet" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/play/agent-wallet
...
Route: /play/agent-wallet ("Agent Wallet (x402 on Solana)", section "crypto").
Declared purpose: Watch your 3D avatar walk up to a paid endpoint and pay for it with its agent wallet: a real USDC micropayment signed by the agent and settled on Solana.
...
1. Load http://localhost:3000/play/agent-wallet. Record the HTTP status, every console
...
- [ ] Local load of /play/agent-wallet: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-play-agent-wallet.md
