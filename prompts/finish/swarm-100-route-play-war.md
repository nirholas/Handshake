# Route audit: /play/war (Coin Wars arena: two communities, one battlefield)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/play/war')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/play/war" in vercel.json (the live
...
Route: /play/war ("Coin Wars arena: two communities, one battlefield", section "crypto").
Declared purpose: The live Coin Wars arena. Enlist at your coin world's war portal, fight the rival community to the kill cap, and the result lands on the Elo ladder. You fight for the coin you hold.
...
1. Load http://localhost:3000/play/war. Record the HTTP status, every console
...
- [ ] Local load of /play/war: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-play-war.md
