...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/irl')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/irl" in vercel.json (the live
...
Route: /irl ("IRL", section "main").
Declared purpose: Place a 3D avatar in your real environment. Camera AR passthrough, joystick movement, and tap-to-place 3D objects on your floor.
...
1. Load http://localhost:3000/irl. Record the HTTP status, every console
...
- [ ] Local load of /irl: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-irl.md
