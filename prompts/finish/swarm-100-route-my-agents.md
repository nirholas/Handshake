# Route audit: /my-agents (My Agents)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/my-agents')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/my-agents" in vercel.json (the live
...
Route: /my-agents ("My Agents", section "agent-tools").
Declared purpose: Your private collection of agents and avatars. Requires sign-in.
...
1. Load http://localhost:3000/my-agents. Record the HTTP status, every console
...
- [ ] Local load of /my-agents: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-my-agents.md
