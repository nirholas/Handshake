...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/radar')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/radar" in vercel.json (the live
...
Route: /radar ("Coin Radar", section "crypto").
Declared purpose: Live pump.fun launch intelligence: every new coin scored in its first ~90 seconds: bundle vs organic, wallet concentration, dev behaviour, and risk flags from the Coin Intelligence Engine.
...
1. Load http://localhost:3000/radar. Record the HTTP status, every console
...
- [ ] Local load of /radar: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-radar.md
