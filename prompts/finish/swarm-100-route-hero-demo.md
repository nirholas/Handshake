# Route audit: /hero-demo (Hero Stage)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/hero-demo')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/hero-demo" in vercel.json (the live
...
Route: /hero-demo ("Hero Stage", section "crypto").
Declared purpose: A cinematic 3D hero stage for three.ws agents: a lit avatar centered in deep space over concentric glowing rings, a drifting starfield, and soft bloom, with pointer parallax and a live avatar switcher. Motion-safe.
...
1. Load http://localhost:3000/hero-demo. Record the HTTP status, every console
...
- [ ] Local load of /hero-demo: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-hero-demo.md
