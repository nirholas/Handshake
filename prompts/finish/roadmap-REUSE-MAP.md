...
| The renderers' three.js supply | `api/_lib/three-cdn.js` | n/a | **[wired]** | All three renderers build a page whose import map points at a pinned three.js release, and they resolve the host through `resolveThreeCdn()` (unpkg, falling back to jsDelivr on a bounded HEAD probe, cached ten minutes). `THREE_VERSION` lives in that one module. Do not hardcode a CDN host in a renderer; `tests/api/three-cdn-wiring.test.js` fails the build if you do. |
| GLB→PNG, pure JS | `poppygl` | ✅ MIT | **[open]** | Software rasterizer, zero native deps. Its original selling point was surviving a Vercel function, which no longer constrains us. Still the cheapest option if a lane ever needs a poster without paying chromium's cold start. |
