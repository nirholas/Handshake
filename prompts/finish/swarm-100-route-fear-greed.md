# Route audit: /fear-greed (Crypto Fear & Greed Index)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/fear-greed')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/fear-greed" in vercel.json (the live
...
Route: /fear-greed ("Crypto Fear & Greed Index", section "crypto").
Declared purpose: The Crypto Fear & Greed Index: live market sentiment from 0 (Extreme Fear) to 100 (Extreme Greed), shown on a gauge with a week-over-week delta, a full interactive history chart, and a labelled scale explaining what each reading means.
...
1. Load http://localhost:3000/fear-greed. Record the HTTP status, every console
...
- [ ] Local load of /fear-greed: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-fear-greed.md
