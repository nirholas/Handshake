...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/hydrate')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/hydrate" in vercel.json (the live
...
Route: /hydrate ("Hydrate", section "build").
Declared purpose: Take an existing on-chain agent (ERC-8004 / Solana) and attach a 3D body, voice, and skills.
...
1. Load http://localhost:3000/hydrate. Record the HTTP status, every console
...
- [ ] Local load of /hydrate: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-hydrate.md
