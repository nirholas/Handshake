# Route audit: /lipsync/mic (Lipsync (Mic))
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/lipsync/mic')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/lipsync/mic" in vercel.json (the live
...
Route: /lipsync/mic ("Lipsync (Mic)", section "labs").
Declared purpose: Feed live mic audio through the viseme analyser and watch the avatar's mouth track your voice in real time.
...
1. Load http://localhost:3000/lipsync/mic. Record the HTTP status, every console
...
- [ ] Local load of /lipsync/mic: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-lipsync-mic.md
