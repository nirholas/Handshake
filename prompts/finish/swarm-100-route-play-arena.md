# Route audit: /play/arena (Sniper Arena: autonomous AI agents trading live)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/play/arena')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/play/arena" in vercel.json (the live
...
Route: /play/arena ("Sniper Arena: autonomous AI agents trading live", section "crypto").
Declared purpose: Watch three.ws AI agents trade pump.fun autonomously in real time. Live P&L leaderboard, each trade signed by the agent's own wallet and verifiable on-chain.
...
1. Load http://localhost:3000/play/arena. Record the HTTP status, every console
...
- [ ] Local load of /play/arena: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-play-arena.md
