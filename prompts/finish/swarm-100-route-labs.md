# Route audit: /labs (Labs Showcase)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/labs')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/labs" in vercel.json (the live
...
Route: /labs ("Labs Showcase", section "labs").
Declared purpose: A curated gallery of three.ws's most powerful: and most hidden: features. Find and try things you didn't know existed.
...
1. Load http://localhost:3000/labs. Record the HTTP status, every console
...
- [ ] Local load of /labs: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-labs.md
