# Route audit: /features/studio (Studio: Embeddable AI Widget Builder)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/features/studio')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/features/studio" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/features/studio
...
Route: /features/studio ("Studio: Embeddable AI Widget Builder", section "build").
Declared purpose: Configure avatar, voice, and knowledge base in the Widget Studio, then copy one script tag to embed a 3D AI agent anywhere.
...
1. Load http://localhost:3000/features/studio. Record the HTTP status, every console
...
- [ ] Local load of /features/studio: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-features-studio.md
