# Route audit: /dashboard (Dashboard)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/dashboard')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/dashboard" in vercel.json (the live
...
Route: /dashboard ("Dashboard", section "account").
Declared purpose: Your account dashboard: agents, avatars, payments, keys, MCP servers, monetization, billing.
...
1. Load http://localhost:3000/dashboard. Record the HTTP status, every console
...
- [ ] Local load of /dashboard: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-dashboard.md
