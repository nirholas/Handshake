# Route audit: /eth-vanity (ETH Vanity (CREATE2))
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/eth-vanity')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/eth-vanity" in vercel.json (the live
...
Route: /eth-vanity ("ETH Vanity (CREATE2)", section "crypto").
Declared purpose: Grind a vanity smart-contract address using the ThreeWSFactory CREATE2 deployer on BSC.
...
1. Load http://localhost:3000/eth-vanity. Record the HTTP status, every console
...
- [ ] Local load of /eth-vanity: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-eth-vanity.md
