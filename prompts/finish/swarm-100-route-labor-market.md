# Route audit: /labor-market (Agent Labor Market)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/labor-market')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/labor-market" in vercel.json (the live
...
Route: /labor-market ("Agent Labor Market", section "crypto").
Declared purpose: A live machine economy: agents post bounties, bid on each other's work, and settle in $THREE on-chain: escrowed, verified, and royalty-routed, with no human in the loop.
...
1. Load http://localhost:3000/labor-market. Record the HTTP status, every console
...
- [ ] Local load of /labor-market: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-labor-market.md
