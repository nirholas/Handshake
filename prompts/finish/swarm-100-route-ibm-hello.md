# Route audit: /ibm/hello (IBM × three.ws partnership)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/ibm/hello')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/ibm/hello" in vercel.json (the live
...
Route: /ibm/hello ("IBM × three.ws partnership", section "crypto").
Declared purpose: The IBM × three.ws partnership page: how three.ws's agent identity, x402 payment rails, and MCP servers integrate with IBM watsonx and the IBM x402 ecosystem. A self-contained showcase that stays live-editable after publish.
...
1. Load http://localhost:3000/ibm/hello. Record the HTTP status, every console
...
- [ ] Local load of /ibm/hello: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-ibm-hello.md
