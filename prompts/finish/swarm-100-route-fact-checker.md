# Route audit: /fact-checker (Fact Checker)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/fact-checker')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/fact-checker" in vercel.json (the live
...
Route: /fact-checker ("Fact Checker", section "crypto").
Declared purpose: Pay $0.10 per claim. The agent searches across authoritative sources, pays per retrieval via x402, and returns a verdict with cited evidence and a signed attestation.
...
1. Load http://localhost:3000/fact-checker. Record the HTTP status, every console
...
- [ ] Local load of /fact-checker: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-fact-checker.md
