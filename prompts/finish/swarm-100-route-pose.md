# Route audit: /pose (Animation Studio)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/pose')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/pose" in vercel.json (the live
...
Route: /pose ("Animation Studio", section "build").
Declared purpose: Pose any three.ws avatar or the built-in mannequin with FK/IK, keyframe a timeline, and preview the motion live. Export an animated GLB or clip JSON, save animations to your account, play them back across the platform, and sell them for USDC.
...
1. Load http://localhost:3000/pose. Record the HTTP status, every console
...
- [ ] Local load of /pose: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-pose.md
