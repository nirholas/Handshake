# Route audit: /nim-forge (TRELLIS NIM Forge)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/nim-forge')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/nim-forge" in vercel.json (the live
...
Route: /nim-forge ("TRELLIS NIM Forge", section "build").
Declared purpose: Drop in a photo, get a textured 3D model, reconstructed on a self-hosted Microsoft TRELLIS NIM. Single-hop image to GLB.
...
1. Load http://localhost:3000/nim-forge. Record the HTTP status, every console
...
- [ ] Local load of /nim-forge: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-nim-forge.md
