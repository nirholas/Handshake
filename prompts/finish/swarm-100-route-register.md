# Route audit: /register (Create account)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/register')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/register" in vercel.json (the live
...
Route: /register ("Create account", section "account").
Declared purpose: Create a new three.ws account.
...
1. Load http://localhost:3000/register. Record the HTTP status, every console
...
- [ ] Local load of /register: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-register.md
