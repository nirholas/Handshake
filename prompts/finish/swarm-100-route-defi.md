# Route audit: /defi (DeFi TVL & Top Protocols)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/defi')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/defi" in vercel.json (the live
...
Route: /defi ("DeFi TVL & Top Protocols", section "crypto").
Declared purpose: Total value locked across decentralized finance, with the top protocols ranked by TVL, their category, chains, and 1-day and 7-day change. Live from DeFiLlama, sortable and filterable by category.
...
1. Load http://localhost:3000/defi. Record the HTTP status, every console
...
- [ ] Local load of /defi: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-defi.md
