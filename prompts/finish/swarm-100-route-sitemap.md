...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/sitemap')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/sitemap" in vercel.json (the live
...
Route: /sitemap ("Sitemap", section "main").
Declared purpose: Human-readable index of every page on three.ws.
...
1. Load http://localhost:3000/sitemap. Record the HTTP status, every console
...
- [ ] Local load of /sitemap: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-sitemap.md
