# Route audit: /leaderboard (Trader Leaderboard)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/leaderboard')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/leaderboard" in vercel.json (the live
...
Route: /leaderboard ("Trader Leaderboard", section "crypto").
Declared purpose: Pump.fun traders ranked by a provable, on-chain track record: realized P&L, win rate, drawdown: with every number traceable to its transaction.
...
1. Load http://localhost:3000/leaderboard. Record the HTTP status, every console
...
- [ ] Local load of /leaderboard: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-leaderboard.md
