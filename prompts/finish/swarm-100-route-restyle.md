# Route audit: /restyle (Restyle Studio: material presets, AI restyle & variants)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/restyle')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/restyle" in vercel.json (the live
...
Route: /restyle ("Restyle Studio: material presets, AI restyle & variants", section "build").
Declared purpose: Re-skin any 3D model in the browser: apply PBR material presets (chrome, gold, glass, wood…), restyle from a plain-language AI instruction, fan out seeded colorway variants as durable versions, fine-tune metalness and roughness live, and export a validated GLB: non-destructively, with a real revertable version lineage.
...
1. Load http://localhost:3000/restyle. Record the HTTP status, every console
...
- [ ] Local load of /restyle: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-restyle.md
