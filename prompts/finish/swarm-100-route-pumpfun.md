# Route audit: /pumpfun (Pump.fun Stream)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/pumpfun')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/pumpfun" in vercel.json (the live
...
Route: /pumpfun ("Pump.fun Stream", section "crypto").
Declared purpose: Mint and trade pump.fun agent tokens on Solana. Live feed of every launch.
...
1. Load http://localhost:3000/pumpfun. Record the HTTP status, every console
...
- [ ] Local load of /pumpfun: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-pumpfun.md
