# Route audit: /mocap-studio (Mocap Studio)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/mocap-studio')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/mocap-studio" in vercel.json (the live
...
Route: /mocap-studio ("Mocap Studio", section "main").
Declared purpose: Drive a 3D avatar with your webcam: real-time facial motion capture in the browser, no rig and no download.
...
1. Load http://localhost:3000/mocap-studio. Record the HTTP status, every console
...
- [ ] Local load of /mocap-studio: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-mocap-studio.md
