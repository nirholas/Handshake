# Route audit: /launch (Launch a Coin)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/launch')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/launch" in vercel.json (the live
...
Route: /launch ("Launch a Coin", section "crypto").
Declared purpose: Mint a coin for your 3D AI agent in one flow: pick an agent, set the name, symbol, and image, then launch on pump.fun straight from your wallet with an optional three.ws-branded vanity mint.
...
1. Load http://localhost:3000/launch. Record the HTTP status, every console
...
- [ ] Local load of /launch: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-launch.md
