# Route audit: /deployments (On-chain Deployments)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/deployments')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/deployments" in vercel.json (the live
...
Route: /deployments ("On-chain Deployments", section "crypto").
Declared purpose: A live, cross-chain feed of every agent registered on the ERC-8004 Identity Registry: the moment each registration lands on-chain. Real on-chain registrations only, with chain footprint, 3D-avatar and x402 capability stats.
...
1. Load http://localhost:3000/deployments. Record the HTTP status, every console
...
- [ ] Local load of /deployments: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-deployments.md
