# Route audit: /minted (Minted 3D Assets)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/minted')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/minted" in vercel.json (the live
...
Route: /minted ("Minted 3D Assets", section "crypto").
Declared purpose: Live public gallery of every generated 3D avatar minted as a Metaplex Core NFT through three.ws: interactive viewer, baked provenance, and enforced creator royalties on every card.
...
1. Load http://localhost:3000/minted. Record the HTTP status, every console
...
- [ ] Local load of /minted: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-minted.md
