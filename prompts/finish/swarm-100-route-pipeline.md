# Route audit: /pipeline (Recording Pipeline: the data loop, live)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/pipeline')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/pipeline" in vercel.json (the live
...
Route: /pipeline ("Recording Pipeline: the data loop, live", section "crypto").
Declared purpose: One-glance health of the closed loop that turns every pump.fun launch into a trade signal: the launch recorder, signal intel, ground-truth outcomes, Oracle conviction, the wallet reputation graph, trained weights, and the agents trading on the result: every stage, in real time.
...
1. Load http://localhost:3000/pipeline. Record the HTTP status, every console
...
- [ ] Local load of /pipeline: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-pipeline.md
