...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/showcase')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/showcase" in vercel.json (the live
...
Route: /showcase ("Showcase", section "main").
Declared purpose: AI-powered 3D model viewer showcase: drop in a model and explore it live.
...
1. Load http://localhost:3000/showcase. Record the HTTP status, every console
...
- [ ] Local load of /showcase: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-showcase.md
