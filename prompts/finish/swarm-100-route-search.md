...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/search')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/search" in vercel.json (the live
...
Route: /search ("Search", section "main").
Declared purpose: One search across every creation type on three.ws: avatars, agents, 3D models, worlds, and coins: with remixable models remixable straight from the results.
...
1. Load http://localhost:3000/search. Record the HTTP status, every console
...
- [ ] Local load of /search: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-search.md
