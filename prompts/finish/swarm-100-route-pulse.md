...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/pulse')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/pulse" in vercel.json (the live
...
Route: /pulse ("Money Pulse", section "crypto").
Declared purpose: The Money Pulse: a real-time, platform-wide feed of three.ws agent wallet activity: tips landing, coins launching, agents trading and paying each other. Every beat is a real on-chain event.
...
1. Load http://localhost:3000/pulse. Record the HTTP status, every console
...
- [ ] Local load of /pulse: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-pulse.md
