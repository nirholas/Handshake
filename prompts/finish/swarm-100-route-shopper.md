# Route audit: /shopper (Endpoint Shopper)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/shopper')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/shopper" in vercel.json (the live
...
Route: /shopper ("Endpoint Shopper", section "crypto").
Declared purpose: Describe a task and set a budget. An AI agent discovers relevant x402 endpoints via the Bazaar, chains them together, and synthesizes a final answer: paying per API call.
...
1. Load http://localhost:3000/shopper. Record the HTTP status, every console
...
- [ ] Local load of /shopper: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-shopper.md
