# Route audit: /pump-visualizer (Pump Visualizer)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/pump-visualizer')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/pump-visualizer" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/pump-visualizer
...
Route: /pump-visualizer ("Pump Visualizer", section "crypto").
Declared purpose: 3D visualizer of trending pump.fun tokens: live market data rendered as an interactive scene.
...
1. Load http://localhost:3000/pump-visualizer. Record the HTTP status, every console
...
- [ ] Local load of /pump-visualizer: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-pump-visualizer.md
