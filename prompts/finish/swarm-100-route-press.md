...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/press')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/press" in vercel.json (the live
...
Route: /press ("Press kit", section "main").
Declared purpose: Logos, announcement graphics, boilerplate, and usage rules for three.ws. Every asset is downloadable and current, and editorial use needs no permission.
...
1. Load http://localhost:3000/press. Record the HTTP status, every console
...
- [ ] Local load of /press: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-press.md
