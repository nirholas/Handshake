# Route audit: /partners (Partner Ecosystem)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/partners')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/partners" in vercel.json (the live
...
Route: /partners ("Partner Ecosystem", section "main").
Declared purpose: three.ws partner ecosystem: OpenAI, IBM, AWS, Google Cloud, Alibaba Cloud, NVIDIA, Quicknode, HackerNoon. The AI, cloud, and infrastructure programs powering 3D AI agents at enterprise scale.
...
1. Load http://localhost:3000/partners. Record the HTTP status, every console
...
- [ ] Local load of /partners: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-partners.md
