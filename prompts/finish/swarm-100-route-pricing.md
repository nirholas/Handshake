...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/pricing')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/pricing" in vercel.json (the live
...
Route: /pricing ("Pricing", section "main").
Declared purpose: Plans and pricing for three.ws: free forge quota, premium passes, and pay-per-use API surfaces.
...
1. Load http://localhost:3000/pricing. Record the HTTP status, every console
...
- [ ] Local load of /pricing: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/finish/swarm-100-route-pricing.md
