...
- E2E: run the fleet in paper mode against LIVE mainnet data for ≥ 30 minutes; then execute at
  least one REAL testnet-46630 trade through the full pipeline (faucet funds; if funding is
  owner-blocked, full-pipeline simulation with the blocker stated plainly) — evidence in report.
...
- [ ] Real testnet trade through the full pipeline (or explicit funding-blocked status).
      Risk tests green. Kill switch proven (test output).
