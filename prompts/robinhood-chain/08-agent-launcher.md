...
1. **Launch rails** (`src/rails/`) — research first, then implement what is REAL:
   - `noxa` rail: the SDK build located the NOXA factory (`0xD9eC2db5…FCcB`, instant v3 pools,
     no bonding curve — full address in the SDK's `addresses.ts`). Reverse-engineer its `create`
     calldata from a recent real launch tx on Blockscout; implement create + initial-buy. If
     NOXA has an official API/SDK, prefer it and cite it.
   - `odyssey` rail: curve factory `0xEb3FeeD2…5a80` (TokenCreated/Traded/PoolMigrated events)
     — implement only if its create path is provable from verified contracts; otherwise
     document why it's excluded. Never guess calldata.
...
5. **Testnet-first pipeline** — full E2E on 46630: deploy, pool, buy, verify — real tx hashes
   (direct rail at minimum; testnet has no official Uniswap — use the community deployment the
   SDK documents). Mainnet execution gated behind `LIVE=1` + funding; do not launch anything
   real on mainnet during the build (a mainnet token launch is an outward-facing, irreversible
   act — that call is the owner's).
