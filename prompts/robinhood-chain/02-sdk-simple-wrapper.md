...
- Vitest: unit (arg validation, formatting) + live (real `hood.price('AAPL')` on mainnet, real
  testnet `hood.swap` with faucet funds — paste tx hash; if faucet remains owner-blocked, run
  the swap against the community testnet router with whatever funded wallet exists, or state
  plainly that the write E2E is pending funding).
...
- [ ] Testnet swap tx hash in the report (or explicit funding-blocked status); size budget
      printed; `npm pack` clean.
