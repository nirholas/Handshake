...
- Vitest: tool-schema validation tests + a scripted MCP client (SDK's client) exercising every
  ...
  (if faucet funding is still owner-blocked, prove through simulation and say so plainly).
...
- [ ] Real testnet swap through the trading server (hash in report) or explicit funding-blocked
      status with simulation evidence. Spend-cap + confirm-gate tests green.
