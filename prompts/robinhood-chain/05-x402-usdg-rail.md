...
1. **Protocol conformance** — implement x402 `exact` scheme for EVM with USDG on chain 4663:
   verify USDG's EIP-3009/`transferWithAuthorization` or EIP-2612 permit support by READING the
   verified contract on Blockscout first; build settlement on whichever the contract actually
   supports (document the finding — this decision is load-bearing). Testnet twin on 46630.
...
5. **Interop proof** — E2E script: a demo paid endpoint + the client completing a REAL paid
   request with USDG on testnet 46630 (faucet ETH for gas; if no USDG testnet faucet exists,
   deploy a mock— NO: deploy nothing fake. If testnet USDG doesn't exist, run the E2E on
   mainnet with a $0.001 real payment funded from the owner's wallet if creds are present,
   otherwise prove the full flow up to settlement broadcast in simulation (`eth_call`) and mark
   settlement as pending-owner-funding in the report. State plainly which path you took.)
