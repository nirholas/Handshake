# Roadmap build: Phase 4: federation adapter for an existing decentralized compute network
...
appropriate. The livepeer dependency already landed for early
experimentation.

Step 0: confirm the livepeer dependency's state in package.json and any
existing usage (grep livepeer across the repo); survey its current inference
API surface from its own docs.

Build: a working adapter behind an env flag that routes one class of GPU job
(pick the platform's cheapest-to-verify lane) to the external network, with
the same job envelope and verification the platform's own workers use, a
measured comparison (latency, cost, success rate over 20 real jobs against
the free or test tier), and a written recommendation in docs/ops on whether
to expand. If the network's free path cannot execute the job class, prove
the adapter to the request boundary with a full dry run and record exactly
what a funded run needs.
...
- [ ] Adapter behind its flag with the standard envelope; 20-job measured
      comparison (or full dry-run proof plus the named funding gap) in the
      report.
- [ ] Recommendation note landed in docs/ops.
- [ ] Adapter covered by tests; npm test green; check:rules clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p4-compute-federation.md
