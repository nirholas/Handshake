# Sweep: Deploy readiness standing check
...
Run the deploy-preflight subagent checklist without deploying: worktree hygiene (clean:worktrees, report what it would reclaim), build order intact (the build:gcp chain in package.json matches the CLAUDE.md runbook), service-account pins present in every cloudbuild config, and npm run build:gcp completes green in a prepared worktree. The deploy itself stays owner-gated; the deliverable is a repo where the ship is one command.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-deploy-preflight.md
