# Route audit: /watchlist (Watchlist)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/watchlist')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/watchlist" in vercel.json (the live
...
Route: /watchlist ("Watchlist", section "crypto").
Declared purpose: Your privately-stored watchlist of pump.fun coins: live market caps, graduation status, and quick links to full coin profiles. No account required; stored in your browser.
...
1. Load http://localhost:3000/watchlist. Record the HTTP status, every console
...
- [ ] Local load of /watchlist: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-watchlist.md
