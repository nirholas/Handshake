# Route audit: /agent-economy (Agent Economy: Live Demo)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agent-economy')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agent-economy" in vercel.json (the live
...
Route: /agent-economy ("Agent Economy: Live Demo", section "crypto").
Declared purpose: Watch two 3D AI agents transact on-chain. Agent A buys data from Agent B: real SOL, real Solana transaction, live.
...
1. Load http://localhost:3000/agent-economy. Record the HTTP status, every console
...
- [ ] Local load of /agent-economy: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agent-economy.md
