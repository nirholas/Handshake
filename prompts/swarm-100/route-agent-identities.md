# Route audit: /agent-identities (Agent Identity Studio: 3D avatars for AI agents)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agent-identities')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agent-identities" in vercel.json (the live
...
Route: /agent-identities ("Agent Identity Studio: 3D avatars for AI agents", section "agent-tools").
Declared purpose: Every AI agent deserves a face. A brand brief becomes a rigged 3D avatar plus posed studio renders: see the demo identities generated end-to-end by the pipeline.
...
1. Load http://localhost:3000/agent-identities. Record the HTTP status, every console
...
- [ ] Local load of /agent-identities: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agent-identities.md
