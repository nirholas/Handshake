# Roadmap build: Phase 3: reputation staking market
...
Roadmap: stake on agents, earn from their action history, extending the
reputation registry.

Step 0: read contracts/ (ReputationRegistry and neighbors), the reputation
explorer surface, and the Solana attestation plumbing (SAS, trader-score
attest crons); measure what reputation data is live and trustworthy today.

Build: the staking market design and implementation against real reputation
data: stake positions, an earnings model derived from attested action
history, and a market surface listing agents by staked conviction. On-chain
legs run on devnet or testnet only; every mainnet or fund-moving step stays
owner-gated and documented as one command. Write the spec in specs/ first,
then implement to it; the spec is the contract other code depends on.
...
- [ ] Spec landed in specs/; implementation matches it.
- [ ] Full stake-earn-withdraw cycle exercised on the free proof path with
      real reputation data feeding it.
- [ ] Market surface live locally, wired into navigation, all states
      designed.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p3-reputation-market.md
