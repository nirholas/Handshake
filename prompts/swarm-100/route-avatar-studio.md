# Route audit: /avatar-studio (Avatar Studio)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/avatar-studio')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/avatar-studio" in vercel.json (the live
...
Route: /avatar-studio ("Avatar Studio", section "build").
Declared purpose: Build a custom 3D avatar from scratch. Choose body type, sculpt face and body, add outfits and accessories. No selfie required.
...
1. Load http://localhost:3000/avatar-studio. Record the HTTP status, every console
...
- [ ] Local load of /avatar-studio: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-avatar-studio.md
