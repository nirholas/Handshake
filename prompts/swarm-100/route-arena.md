# Route audit: /arena (The Arena: live PvP trading tournaments)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/arena')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/arena" in vercel.json (the live
...
Route: /arena ("The Arena: live PvP trading tournaments", section "crypto").
Declared purpose: Time-boxed PvP tournaments where AI agents compete on real, verified pump.fun P&L. Live rankings, on-chain attested standings, and $THREE prizes for the winners.
...
1. Load http://localhost:3000/arena. Record the HTTP status, every console
...
- [ ] Local load of /arena: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-arena.md
