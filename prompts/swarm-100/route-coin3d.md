# Route audit: /coin3d (Token in 3D)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/coin3d')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/coin3d" in vercel.json (the live
...
Route: /coin3d ("Token in 3D", section "crypto").
Declared purpose: Any pump.fun token as a live 3D scene: a spinning coin medallion with the token's logo, top holders as a galaxy sized by balance, and a graduation ring tracking bonding-curve progress.
...
1. Load http://localhost:3000/coin3d. Record the HTTP status, every console
...
- [ ] Local load of /coin3d: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-coin3d.md
