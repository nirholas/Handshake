...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/clash')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/clash" in vercel.json (the live
...
Route: /clash ("Coin Clash", section "crypto").
Declared purpose: Token-gated community warfare. Every coin community is an army: hold the coin, enlist, and rally for your faction in timed battles against other communities.
...
1. Load http://localhost:3000/clash. Record the HTTP status, every console
...
- [ ] Local load of /clash: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-clash.md
