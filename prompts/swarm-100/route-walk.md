# Route audit: /walk (Walk Anywhere on the Web)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/walk')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/walk" in vercel.json (the live
...
Route: /walk ("Walk Anywhere on the Web", section "main").
Declared purpose: Your 3D avatar walks anywhere on the web. Live demos across six environments, a Chrome extension, one-tag embeds, and a global distance leaderboard.
...
1. Load http://localhost:3000/walk. Record the HTTP status, every console
...
- [ ] Local load of /walk: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-walk.md
