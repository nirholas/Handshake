# Route audit: /sperax (Sperax on three.ws)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/sperax')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/sperax" in vercel.json (the live
...
Route: /sperax ("Sperax on three.ws", section "crypto").
Declared purpose: Free AI credits for three.ws users on chat.sperax.io: what the integration is and how to claim it.
...
1. Load http://localhost:3000/sperax. Record the HTTP status, every console
...
- [ ] Local load of /sperax: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-sperax.md
