# Route audit: /bundles (Skill Bundles)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/bundles')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/bundles" in vercel.json (the live
...
Route: /bundles ("Skill Bundles", section "build").
Declared purpose: Sell several of an agent's skills as one purchase, priced from your own sales instead of a rule of thumb. three.ws finds the buyers who already bought two or more of those skills separately and backtests any price against what they really paid, so you see what the bundle would have earned before you publish it.
...
1. Load http://localhost:3000/bundles. Record the HTTP status, every console
...
- [ ] Local load of /bundles: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-bundles.md
