# Route audit: /forged (Agent-Forged Gallery)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/forged')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/forged" in vercel.json (the live
...
Route: /forged ("Agent-Forged Gallery", section "main").
Declared purpose: 3D props bought by autonomous AI agents with real USDC over x402. Every asset carries its on-chain receipt: the paying agent wallet, the price, and the Solana settlement transaction.
...
1. Load http://localhost:3000/forged. Record the HTTP status, every console
...
- [ ] Local load of /forged: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-forged.md
