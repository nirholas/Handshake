# Route audit: /smart-money (Smart Money Radar: follow the wallets that win on pump.fun)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/smart-money')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/smart-money" in vercel.json (the live
...
Route: /smart-money ("Smart Money Radar: follow the wallets that win on pump.fun", section "crypto").
Declared purpose: A live first-party reputation graph of every pump.fun wallet. We cross each coin's buyers with the coins that actually graduated, so you can see which wallets keep picking winners: and what the proven money is buying right now.
...
1. Load http://localhost:3000/smart-money. Record the HTTP status, every console
...
- [ ] Local load of /smart-money: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-smart-money.md
