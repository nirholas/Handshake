# Roadmap build: Phase 3: audit-readiness pack for the economy contracts
...
Roadmap funding line: multi-firm audits for the reputation, royalty, and
delegation contracts. Engaging a firm is an owner action; everything a firm
needs on day one is buildable now.

Step 0: inventory contracts/ (and crates/ if Solana programs live there):
every contract source, its deploy status, its test coverage measured with
the toolchain's coverage tool, and its documented invariants (or absence).

Build: for each economy contract: a written invariant spec in specs/, a
threat model note (assets, actors, failure modes), test coverage raised to
cover every invariant including negative cases, static analysis run with a
standard OSS analyzer and every finding fixed or explicitly dispositioned,
and a single AUDIT-README in contracts/ that a firm could start from. No
deployments; this order never touches a live chain.
...
- [ ] Invariant specs and threat model landed in specs/ and contracts/.
- [ ] Coverage numbers before and after in the report; every invariant has a
      test including its negative case.
- [ ] Static analysis output committed with each finding fixed or
      dispositioned in writing.
- [ ] npm test (and the contracts toolchain's test command) green;
      check:rules clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p3-contracts-audit-pack.md
