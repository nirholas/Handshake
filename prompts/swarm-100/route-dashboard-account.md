# Route audit: /dashboard/account (Dashboard · Account)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/dashboard/account')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/dashboard/account" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/dashboard/account
...
Route: /dashboard/account ("Dashboard · Account", section "account").
Declared purpose: Manage connected wallets, SNS, delegation, and action log.
...
1. Load http://localhost:3000/dashboard/account. Record the HTTP status, every console
...
- [ ] Local load of /dashboard/account: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-dashboard-account.md
