# Roadmap build: Phase 1: selfie capture UX with realtime quality gates
...
Ship the mobile-first 3-shot capture experience the roadmap promises: left,
center, right selfies with realtime lighting, framing, and blur gates so a
user cannot submit a capture the pipeline will reject.

Step 0: read src/selfie-capture.js and src/selfie-pipeline.js, load /create/selfie
and the /creating test bench locally, and run the existing capture flow on a
phone-sized viewport to measure what already works and where it fails.

Build: per-shot quality scoring surfaced live in the viewfinder (lighting,
face framing, blur), retake prompts that say exactly what is wrong, a
progress model that survives a page refresh mid-capture, and a 320px-first
layout. The gates must reject for the same reasons the reconstruction
pipeline fails, so derive thresholds from real pipeline rejections, not
invented numbers.
...
- [ ] Capture flow exercised end to end in a real mobile-sized browser with
      each gate observed rejecting a genuinely bad shot and passing a good one.
- [ ] Gate thresholds traced to real pipeline failure modes in the report.
- [ ] Zero console errors through the whole flow; states designed for camera
      denied, no camera, and mid-flow refresh.
- [ ] Tests cover the scoring functions; npm test green; check:rules clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p1-capture-quality-gates.md
