# Route audit: /viewer (glTF / GLB Viewer)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/viewer')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/viewer" in vercel.json (the live
...
Route: /viewer ("glTF / GLB Viewer", section "labs").
Declared purpose: Preview any public glTF/GLB inline, then take it further: pose and animate it, restyle the material, build a scene, place it in AR, embed it on your site, or launch it as a pump.fun coin.
...
1. Load http://localhost:3000/viewer. Record the HTTP status, every console
...
- [ ] Local load of /viewer: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-viewer.md
