# Route audit: /concierge (Concierge: AI Chat Widget)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/concierge')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/concierge" in vercel.json (the live
...
Route: /concierge ("Concierge: AI Chat Widget", section "main").
Declared purpose: Give any website an AI concierge with a talking 3D avatar: one-tag embed, streaming answers grounded in the live page, voice in and out, themes, and an avatar picker. Free and open source.
...
1. Load http://localhost:3000/concierge. Record the HTTP status, every console
...
- [ ] Local load of /concierge: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-concierge.md
