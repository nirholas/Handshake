...
memory of what used to fail:
    ...
    npm run verify:x402 && npm run audit:x402-catalog && npm run smoke:x402-facilitator
...
Run all three. Catalog drift gets fixed at the source of truth; facilitator smoke failures get root-caused (the x402-economy-triage agent in .claude/agents/ encodes the known failure map; use it before concluding wallets are dry). No real settlements: verification paths only.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-x402.md
