# Route audit: /cosmos (Cosmos: Living Worlds)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/cosmos')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/cosmos" in vercel.json (the live
...
Route: /cosmos ("Cosmos: Living Worlds", section "build").
Declared purpose: Bring your avatar to life: type a world and NVIDIA Cosmos renders a living, photoreal scene that plays behind your 3D avatar. Generate, preview, and download a short cinematic clip.
...
1. Load http://localhost:3000/cosmos. Record the HTTP status, every console
...
- [ ] Local load of /cosmos: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-cosmos.md
