# Roadmap build: Phase 4: on-chain settlement for inference jobs
...
Roadmap: pay-per-token settlement with cryptographic receipts for inference
work.

Step 0: read the x402 settlement path and receipt plumbing the platform
already runs (docs/x402-revenue, the receipts surfaces); measure what can be
reused: the settlement rail exists, inference metering does not.

Build: per-job metering (tokens in and out) bound to the job's signed
response, settlement over the existing x402 rail on its test lane, and a
receipt format that cryptographically ties payment to the exact job and
response signature. Verification tooling so a node operator can prove they
were paid for exactly the work performed. No real funds in proof runs;
mainnet activation documented and owner-gated.
...
- [ ] One inference job metered, settled on the test lane, and its receipt
      verified against the job signature; transcript in the report.
- [ ] Receipt format documented in specs/.
- [ ] Settlement and verification covered by tests; npm test green;
      check:rules clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p4-inference-settlement.md
