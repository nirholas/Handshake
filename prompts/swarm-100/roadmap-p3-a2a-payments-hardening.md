# Roadmap build: Phase 3: agent-to-agent payments hardened for autonomy
...
Roadmap: agents transact autonomously via delegated signer wallets. The
plumbing exists; autonomy needs hard guarantees around it.

Step 0: read the agent wallet API, spending-envelope tutorial code, the
transaction guard in the agent runtime docs, and the x402 ring economy
docs; run the existing agent-wallet smoke (npm run smoke:agent-wallet) and
record what protections are real today.

Build: enforceable per-agent spend policies (caps per call, per day, per
counterparty), receipts for every agent-initiated payment queryable from the
agent's ledger surface, a kill switch that verifiably halts an agent's
spending, and tests that prove each limit actually blocks an over-limit
attempt on the test lane. No real funds in proof runs.
...
- [ ] Each policy limit demonstrated blocking an over-limit attempt on the
      test lane; transcripts in the report.
- [ ] Kill switch exercised and verified to halt spending immediately.
- [ ] Receipts queryable per agent on a live local surface.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p3-a2a-payments-hardening.md
