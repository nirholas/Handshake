# API audit: api/cron batch 04 (10 files)
...
1. Confirm each file below still exists; deleted ones are done, skip them.
2. For each, decide routability: grep its path in vercel.json and read how
   server/index.mjs maps filesystem handlers. Routed files get an endpoint
   audit; internal modules get a code and test audit.
3. Start a local server for endpoint checks: node server/index.mjs with .env
   loaded, on a free PORT if 3000 is busy.

## Task: audit these api files, then fix everything found

Files in this batch:

- api/cron/irl-reap.js
- api/cron/kol-tracker-refresh.js
- api/cron/launcher-claimer.js
- api/cron/launcher-tick.js
- api/cron/leaderboard-rollup.js
- api/cron/mirror-fanout.js
- api/cron/news-archive-append.js
- api/cron/oracle-calibrate.js
- api/cron/oracle-digest.js
- api/cron/oracle-realized-labels.js

For every routed handler:

1. Exercise it with curl against the local server: the success path returns
   real data with the correct status; malformed input returns a 4xx JSON
   error, never a stack trace or an HTML error page; auth is enforced where
   the code intends it; CORS matches neighboring handlers.
2. Hard-rule scan: no mock fallbacks, no fake-data arrays, no stubbed
   responses, no unfinished markers. Errors are handled at the boundary and
   internal code trusts itself.
3. Do not spend funds or sign anything on-chain while probing. Payment and
   trading handlers get their validation and error paths exercised, never a
   live settlement.

For every internal (non-routed) module:

4. Something must import it; if nothing does, delete it as dead code (verify
   with a repo-wide grep first).
5. Its main path and one failure path must be covered in tests/; add the
   missing test if not.

## Definition of done

- [ ] Every listed file has recorded evidence: a curl transcript for routed
      handlers, a passing test or a deletion for internal modules.
- [ ] Zero hard-rule violations remain in these files.
- [ ] vitest run passes; npm run check:rules -- --paths <touched files>
      exits 0.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/api-cron-04.md
