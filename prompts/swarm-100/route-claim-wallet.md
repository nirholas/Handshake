# Route audit: /claim-wallet (Claim Your Wallet: Verified pump.fun Track Record)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/claim-wallet')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/claim-wallet" in vercel.json (the live
...
Route: /claim-wallet ("Claim Your Wallet: Verified pump.fun Track Record", section "crypto").
Declared purpose: Paste your Solana wallet to see your provable pump.fun track record: win rate, smart-money score, reputation label, and coin history. Sign in to claim it as your official three.ws Trader Card.
...
1. Load http://localhost:3000/claim-wallet. Record the HTTP status, every console
...
- [ ] Local load of /claim-wallet: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-claim-wallet.md
