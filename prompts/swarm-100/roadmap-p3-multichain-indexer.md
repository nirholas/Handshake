# Roadmap build: Phase 3: agent token and reputation indexer, Solana first
...
Roadmap funding line: indexer infrastructure across chains. Build the
platform's own index of agent tokens and reputation events, Solana first,
Base as the first EVM leg.

Step 0: read the existing crawl crons (erc8004-crawl, solana-agents-crawl,
solana-attestations-crawl) and their stores; measure coverage, lag, and
gaps against live chain data for a sample of known agents.

Build: close the coverage gaps the measurement finds, add the missing event
classes (token launches, reputation attestations, transfers) to the crawl,
persist with absolute timestamps, expose the index through the existing API
surfaces that render agent history, and add lag monitoring so staleness is
visible on the status surface. Free RPC lanes and the existing provider
failover chain only; no new paid API without approval.
...
- [ ] Measured before and after coverage for the sampled agents in the
      report.
- [ ] New event classes crawled, stored, and rendered on agent surfaces.
- [ ] Index lag visible on the status surface with a real number.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p3-multichain-indexer.md
