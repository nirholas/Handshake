# Route audit: /create/prompt (Describe it to 3D)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/create/prompt')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/create/prompt" in vercel.json (the live
...
Route: /create/prompt ("Describe it to 3D", section "build").
Declared purpose: Type a description. About a minute later: a rigged 3D avatar you can animate and download.
...
1. Load http://localhost:3000/create/prompt. Record the HTTP status, every console
...
- [ ] Local load of /create/prompt: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-create-prompt.md
