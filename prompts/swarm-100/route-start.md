...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/start')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/start" in vercel.json (the live
...
Route: /start ("Get Started", section "build").
Declared purpose: 5-step onboarding wizard: create a 3D avatar, name your agent, enable skills, deploy an embed widget, and set up monetization: all in under 5 minutes.
...
1. Load http://localhost:3000/start. Record the HTTP status, every console
...
- [ ] Local load of /start: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-start.md
