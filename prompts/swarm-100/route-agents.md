# Route audit: /agents (Agents Index)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agents')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agents" in vercel.json (the live
...
Route: /agents ("Agents Index", section "agent-tools").
Declared purpose: Browse every registered agent on three.ws.
...
1. Load http://localhost:3000/agents. Record the HTTP status, every console
...
- [ ] Local load of /agents: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agents.md
