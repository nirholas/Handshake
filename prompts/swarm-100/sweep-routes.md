# Sweep: Route table audits clean
...
memory of what used to fail:
    ...
    npm run audit:routes && npm run audit:route-shadowing && npm run audit:handlers
...
Run all three. Fix unreachable routes, shadowed routes, and handler mismatches at the route table or handler, whichever is wrong. vercel.json is live config that server/index.mjs boots from; treat edits to it as production code.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-routes.md
