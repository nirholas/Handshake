# Roadmap build: Phase 2: agent memory seeding from Farcaster with explicit consent
...
Same deliverable as the other seeding orders, for Farcaster, independent of
both: build to the existing memory interfaces.

Step 0: read packages/agent-memory and survey OSS Farcaster clients (search
npm for maintained hub or API clients before writing protocol code; log the
choice and license in the report).

Build: consent-first Farcaster connect (wallet-signature verification of the
fid, or the least-privilege method the surveyed client supports), ingest of
the user's casts into agent memory, revocation deletes. Solana-native wallet
UX leads where a wallet is involved.
...
- [ ] Connect, consent, seed, converse, revoke, verify-deleted exercised end
      to end against a real Farcaster account.
- [ ] OSS client choice and license documented in the report.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p2-memory-seed-farcaster.md
