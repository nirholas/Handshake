...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/tour')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/tour" in vercel.json (the live
...
Route: /tour ("Guided Tour", section "main").
Declared purpose: A 3D guide walks across the live site, points at every feature, and narrates what each one does and how to use it: voice plus an on-screen chat bubble, paced by you.
...
1. Load http://localhost:3000/tour. Record the HTTP status, every console
...
- [ ] Local load of /tour: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-tour.md
