# Route audit: /artifact (Artifact Viewer)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/artifact')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/artifact" in vercel.json (the live
...
Route: /artifact ("Artifact Viewer", section "build").
Declared purpose: Renders Claude artifact bundles as standalone embeddable apps.
...
1. Load http://localhost:3000/artifact. Record the HTTP status, every console
...
- [ ] Local load of /artifact: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-artifact.md
