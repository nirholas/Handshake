...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/credits')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/credits" in vercel.json (the live
...
Route: /credits ("Credits", section "account").
Declared purpose: Top up with SOL or $THREE and spend credits on 3D generation, agents, and compute. Hold $THREE for up to 30% off every spend.
...
1. Load http://localhost:3000/credits. Record the HTTP status, every console
...
- [ ] Local load of /credits: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-credits.md
