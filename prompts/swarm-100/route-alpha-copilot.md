# Route audit: /alpha-copilot (Alpha Co-pilot: your agent narrates its own alpha)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/alpha-copilot')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/alpha-copilot" in vercel.json (the live
...
Route: /alpha-copilot ("Alpha Co-pilot: your agent narrates its own alpha", section "main").
Declared purpose: Your 3D agent reads a real pump.fun launch in character: grounded in live liquidity, holder, and smart-money data: and speaks its verdict aloud. Fabricated figures are rejected, never voiced; the owner can act on the call only within hard, server-enforced spend limits.
...
1. Load http://localhost:3000/alpha-copilot. Record the HTTP status, every console
...
- [ ] Local load of /alpha-copilot: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-alpha-copilot.md
