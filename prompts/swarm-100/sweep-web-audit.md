# Sweep: Unauthenticated page sweep clean
...
Run the unauthenticated production page sweep. Every failing page gets a root-cause fix committed. If a failure only reproduces in production and the fix needs a deploy, commit the fix and flag the deploy in the report.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-web-audit.md
