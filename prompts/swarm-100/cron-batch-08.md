# Cron audit batch 08: /api/cron/run-x-triggers and 7 more
...
2. Scheduler truth (crons run on Cloud Scheduler, never in-process):
       ...
       gcloud scheduler jobs list --project aerial-vehicle-466722-p5 --location us-central1 | grep -E 'run-x-triggers|fetch-x-metrics|pumpfun-signals|pump-agent-stats|three-holders-snapshot|recompute-reputation|custody-attest|pumpfun-monitor'
...
- /api/cron/run-x-triggers
- /api/cron/fetch-x-metrics
- /api/cron/pumpfun-signals
- /api/cron/pump-agent-stats
- /api/cron/three-holders-snapshot
- /api/cron/recompute-reputation
- /api/cron/custody-attest
- /api/cron/pumpfun-monitor
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/cron-batch-08.md
