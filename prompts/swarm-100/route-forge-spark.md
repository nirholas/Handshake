# Route audit: /forge-spark (Text → 3D: Nemotron → FLUX → TRELLIS)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/forge-spark')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/forge-spark" in vercel.json (the live
...
Route: /forge-spark ("Text → 3D: Nemotron → FLUX → TRELLIS", section "build").
Declared purpose: Describe an object and watch it become a textured 3D mesh: Nemotron sharpens the prompt, FLUX paints a clean reference, and TRELLIS reconstructs the model: served on NVIDIA NIM, quiet and always on.
...
1. Load http://localhost:3000/forge-spark. Record the HTTP status, every console
...
- [ ] Local load of /forge-spark: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-forge-spark.md
