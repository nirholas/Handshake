# Route audit: /agenc/embodied (AgenC · Embodied)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agenc/embodied')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agenc/embodied" in vercel.json (the live
...
Route: /agenc/embodied ("AgenC · Embodied", section "main").
Declared purpose: Watch two AI agents negotiate and coordinate a task, embodied as 3D characters: the AgenC coordination protocol made visible.
...
1. Load http://localhost:3000/agenc/embodied. Record the HTTP status, every console
...
- [ ] Local load of /agenc/embodied: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agenc-embodied.md
