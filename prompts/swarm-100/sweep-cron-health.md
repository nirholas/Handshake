# Sweep: Cron drift and liveness clean
...
memory of what used to fail:
    ...
    npm run check:cron-drift && npm run check:cron-syntax && npm run audit:cron-liveness
...
Run all three. Drift between vercel.json crons and Cloud Scheduler gets reconciled; syntax failures get fixed; liveness failures (a cron that never fires or always errors) get root-caused in the handler or the schedule.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-cron-health.md
