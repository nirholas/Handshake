...
Run the full suite (vitest then Playwright; the Playwright stage is gated on vitest passing). Fix every failure at its root: the code under test when the code is wrong, the test when the test asserts stale behavior. Never pipe through tail, never skip or quarantine a test to get to green.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-tests.md
