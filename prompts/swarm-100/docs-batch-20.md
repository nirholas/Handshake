# Docs audit batch 20: /docs/agent-runtime through /cookbook/parallel-asset-pack
...
3. Probe production for each path:
       ...
       for p in /docs/agent-runtime /docs/free-llm-providers /docs/nvidia-inception/index.html /docs/nvidia-nemotron-spotlight /cookbook /cookbook/self-correcting-3d /cookbook/text-to-3d-cli /cookbook/parallel-asset-pack; do printf '%s ' "$p"; curl -so /dev/null -w '%{http_code}\n' "https://three.ws$p"; done

## Task: audit this batch of docs pages, then fix everything found

Pages in this batch:

- /docs/agent-runtime ("Docs · The agent runtime and transaction guard")
- /docs/free-llm-providers ("Docs · The free LLM failover chain")
- /docs/nvidia-inception/index.html ("Docs · NVIDIA Inception membership overview")
- /docs/nvidia-nemotron-spotlight ("Docs · How Nemotron Nano made free text-to-3D production-ready")
- /cookbook ("Cookbook: runnable recipes for the 3D API")
- /cookbook/self-correcting-3d ("Recipe: a self-correcting 3D collectible set")
- /cookbook/text-to-3d-cli ("Recipe: text to 3D from the command line")
- /cookbook/parallel-asset-pack ("Recipe: build a whole asset pack in parallel")
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/docs-batch-20.md
