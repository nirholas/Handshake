# Cron audit batch 01: /api/cron/economy-tick and 7 more
...
2. Scheduler truth (crons run on Cloud Scheduler, never in-process):
       ...
       gcloud scheduler jobs list --project aerial-vehicle-466722-p5 --location us-central1 | grep -E 'economy-tick|payment-session-sweep|three-market-refresh|avaturn-seed-cron|irl-drops-refund|flush-usage-events|wallet-intents|reconcile-decisions'
...
- /api/cron/payment-session-sweep
- /api/cron/three-market-refresh
- /api/cron/avaturn-seed-cron
- /api/cron/irl-drops-refund
- /api/cron/flush-usage-events
- /api/cron/wallet-intents
- /api/cron/reconcile-decisions
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/cron-batch-01.md
