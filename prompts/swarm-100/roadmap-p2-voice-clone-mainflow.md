# Roadmap build: Phase 2: voice cloning integrated into the main create flow
...
Roadmap: voice clone shipped behind /demos; main-flow integration is the gap.
Move it into the primary agent creation flow.

Step 0: find the demo implementation (grep voice clone across src/, api/,
packages/voice), read /voice and the create flow, and run the demo path to
see what works. Known constraint: there is no platform ELEVENLABS_API_KEY
anywhere in prod, so the working lane is bring-your-own-key, and owner rule
requires vendor-billed features be metered to $THREE credits or BYOK.

Build: a voice step in the create flow where a user records or uploads a
short sample and binds the resulting voice to their agent. BYOK is the
activation path today: design the key entry, storage, and per-agent binding
properly. Wire the platform-key lane fully behind its env var so one
credential activates it. The agent's voice must actually play in the chat
and embed surfaces once bound.
...
- [ ] Voice binding exercised end to end with a real key (BYOK) and the bound
      voice heard on the agent's chat surface.
- [ ] Platform-key lane fully wired behind ELEVENLABS_API_KEY and proven with
      a dry run; the single missing var named in the report.
- [ ] Designed states for no key, bad sample, and provider failure.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p2-voice-clone-mainflow.md
