# Route audit: /communities (Coin Communities)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/communities')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/communities" in vercel.json (the live
...
Route: /communities ("Coin Communities", section "crypto").
Declared purpose: Pick a coin, drop in an avatar, and walk around its world with everyone else holding it. Each coin is its own live 3D community.
...
1. Load http://localhost:3000/communities. Record the HTTP status, every console
...
- [ ] Local load of /communities: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-communities.md
