...
Run the full repo gate. Fix every failing stage at its root cause, in order of appearance; never mask a check or skip a stage. The gate chains conflict, claude-md, cron, MCP, route, handler, page, docs, tutorial, guard, token, and tour audits; each failure names its own fix.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-gate.md
