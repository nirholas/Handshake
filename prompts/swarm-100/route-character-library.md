# Route audit: /character-library (Character Library)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/character-library')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/character-library" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/character-library
...
Route: /character-library ("Character Library", section "main").
Declared purpose: 106 professionally rigged 3D characters, free to use. Every one drives the full animation library and drops into the Studio, Animation Studio, and embed viewer in one click.
...
1. Load http://localhost:3000/character-library. Record the HTTP status, every console
...
- [ ] Local load of /character-library: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-character-library.md
