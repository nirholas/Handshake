# Roadmap build: Phase 1: automated likeness scoring harness
...
The phase 1 verification metric is a likeness score. Build the automated
harness that scores a reconstruction against its input selfies so quality is
measured, not asserted.

Step 0: read the existing quality tooling (quality-bench cron, any eval code
under scripts/ or workers/) and the avatar-fidelity docs; measure what
scoring exists today.

Build: a scoring pipeline that renders the produced avatar head-on and
profile, compares against the input captures with a real face-embedding
model (open source first: search npm and GitHub before writing any scoring
math), stores per-generation scores alongside the generation record, and
exposes the distribution on an internal surface. Wire it into the existing
quality-bench cron cadence.
...
- [ ] Ten real reconstructions scored; scores stored with their generation
      records and the distribution visible on an internal surface.
- [ ] Scoring model choice justified in the report (OSS survey, license).
- [ ] Cron wiring verified with a clean run; npm test green; check:rules
      clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p1-likeness-eval.md
