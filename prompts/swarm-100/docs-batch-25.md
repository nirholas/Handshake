# Docs audit batch 25: /docs/walk-leaderboard through /tutorials/sell-a-skill-with-a-trial
...
3. Probe production for each path:
       ...
       for p in /docs/walk-leaderboard /docs/atlas /docs/guards /tutorials/wallet-sign-in /tutorials/agent-spending-envelope /tutorials/render-avatar-images /tutorials/character-library-to-embed /tutorials/sell-a-skill-with-a-trial; do printf '%s ' "$p"; curl -so /dev/null -w '%{http_code}\n' "https://three.ws$p"; done

## Task: audit this batch of docs pages, then fix everything found

Pages in this batch:

- /docs/walk-leaderboard ("Docs: Walk Leaderboard")
- /docs/atlas ("Docs: Atlas and Cmd+K search")
- /docs/guards ("Repository guards")
- /tutorials/wallet-sign-in ("Tutorial · Add Wallet Sign-In to Your App")
- /tutorials/agent-spending-envelope ("Tutorial · Give an Agent a Spending Envelope")
- /tutorials/render-avatar-images ("Tutorial · Render Avatars as Images")
- /tutorials/character-library-to-embed ("Tutorial · Character Library to a Live Embed")
- /tutorials/sell-a-skill-with-a-trial ("Tutorial · Sell a Skill With a Free Trial That Converts")
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/docs-batch-25.md
