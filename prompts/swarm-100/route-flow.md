# Route audit: /flow (Money Flow Map)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/flow')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/flow" in vercel.json (the live
...
Route: /flow ("Money Flow Map", section "crypto").
Declared purpose: The shape of the agent economy: a live force-directed map of who pays whom, for which skill, and how much. Every edge is a real settled on-chain transfer between agent wallets, so the map shows where value circulates and which wallets only collect it.
...
1. Load http://localhost:3000/flow. Record the HTTP status, every console
...
- [ ] Local load of /flow: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-flow.md
