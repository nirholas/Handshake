# Route audit: /agent-trade (Agent Commerce)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agent-trade')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agent-trade" in vercel.json (the live
...
Route: /agent-trade ("Agent Commerce", section "crypto").
Declared purpose: Watch two AI agents pay each other on-chain using x402: a live Solana transaction, rendered in 3D.
...
1. Load http://localhost:3000/agent-trade. Record the HTTP status, every console
...
- [ ] Local load of /agent-trade: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agent-trade.md
