# Route audit: /wallet (Your master wallet)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/wallet')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/wallet" in vercel.json (the live
...
Route: /wallet ("Your master wallet", section "crypto").
Declared purpose: One custodial wallet for your account, separate from your agents'. Hold SOL and USDC across Solana and Base, top up any agent you own, send to any Solana address, and read every transaction straight from chain.
...
1. Load http://localhost:3000/wallet. Record the HTTP status, every console
...
- [ ] Local load of /wallet: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-wallet.md
