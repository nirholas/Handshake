# Docs audit batch 37: /docs/package-extraction through /tutorials/getting-started
...
3. Probe production for each path:
       ...
       for p in /docs/package-extraction /tutorials/find-a-better-exit-policy /tutorials/read-the-trading-fleet /docs/live-steps /docs/mcp-safety /mcp-tools /docs/trading-experiment /tutorials/getting-started; do printf '%s ' "$p"; curl -so /dev/null -w '%{http_code}\n' "https://three.ws$p"; done

## Task: audit this batch of docs pages, then fix everything found

Pages in this batch:

- /docs/package-extraction ("Docs · Package extraction")
- /tutorials/find-a-better-exit-policy ("Tutorial · Find a better exit policy")
- /tutorials/read-the-trading-fleet ("Tutorial · Read the trading fleet")
- /docs/live-steps ("Docs · Live steps: documentation that runs")
- /docs/mcp-safety ("Docs · MCP tool safety")
- /mcp-tools ("MCP Tool Catalog")
- /docs/trading-experiment ("Docs · The 10 SOL trading experiment")
- /tutorials/getting-started ("Tutorial · Getting started (hosted)")
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/docs-batch-37.md
