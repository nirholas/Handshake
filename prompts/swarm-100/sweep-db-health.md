# Sweep: Database migrations and generation ground truth
...
Read migration state (db:status previews; db:migrate applies immediately with no dry run, so never run it without reading db:status first). Then query forge_creations for the last 7 days of generation errors, rank error classes by frequency, and root-cause the top recurring class in the pipeline code. Fix and commit.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-db-health.md
