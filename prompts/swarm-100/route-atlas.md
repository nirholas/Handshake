...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/atlas')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/atlas" in vercel.json (the live
...
Route: /atlas ("Atlas", section "main").
Declared purpose: The whole platform as one searchable map: every page grouped by what it is for, plus the task shortcuts the Cmd+K palette answers with. Press Cmd+K anywhere on the site to search it without leaving the page.
...
1. Load http://localhost:3000/atlas. Record the HTTP status, every console
...
- [ ] Local load of /atlas: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-atlas.md
