# Sweep: Production smoke sweep clean
...
Run the production page sweep (every page declared in data/pages.json against the live site). Every failure gets a committed root-cause fix; failures that need a deploy to clear get flagged in the report with the deploy left to the owner.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-smoke-prod.md
