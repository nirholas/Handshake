# Machine surface audit: sitemap, robots, llms.txt, openapi
...
## Step 0: re-derive the current state
    ...
    curl -so /dev/null -w '/sitemap.xml %{http_code}\n' https://three.ws/sitemap.xml
    curl -so /dev/null -w '/llms.txt %{http_code}\n' https://three.ws/llms.txt
    curl -so /dev/null -w '/llms-full.txt %{http_code}\n' https://three.ws/llms-full.txt
    curl -so /dev/null -w '/robots.txt %{http_code}\n' https://three.ws/robots.txt
    curl -so /dev/null -w '/openapi.json %{http_code}\n' https://three.ws/openapi.json

## Task: verify every crawler and agent feed

Endpoints: /sitemap.xml, /llms.txt, /llms-full.txt, /robots.txt, /openapi.json

1. /sitemap.xml is valid XML and its URL set matches data/pages.json (the
   build pipeline derives it; a mismatch means a stale build artifact or a
   generator bug; fix the generator side).
2. /robots.txt allows what the platform wants crawled and references the
   sitemap.
3. /llms.txt and /llms-full.txt parse, and every path they list resolves.
4. /openapi.json parses as valid OpenAPI; spot-check ten declared operations
   against the live api/ handlers (method, path, auth) and fix drift at the
   generator or handler, whichever is wrong.

## Definition of done

- [ ] All endpoints return 200 with valid content.
- [ ] Sitemap matches data/pages.json exactly.
- [ ] Every path listed in llms.txt and llms-full.txt resolves.
- [ ] Ten openapi operations spot-checked against real handlers, drift fixed.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/machine-feeds.md
