# Route audit: /asl-alphabet (ASL Alphabet)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/asl-alphabet')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/asl-alphabet" in vercel.json (the live
...
Route: /asl-alphabet ("ASL Alphabet", section "main").
Declared purpose: The American manual alphabet on a live 3D avatar: every letter and number, what to look for, the look-alikes it is confused with, spell any word, and a drill for reading fingerspelling.
...
1. Load http://localhost:3000/asl-alphabet. Record the HTTP status, every console
...
- [ ] Local load of /asl-alphabet: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-asl-alphabet.md
