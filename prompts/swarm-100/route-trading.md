# Route audit: /trading (Autonomous Trading)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/trading')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/trading" in vercel.json (the live
...
Route: /trading ("Autonomous Trading", section "crypto").
Declared purpose: The control room for three.ws autonomous trading: live fleet vitals, the agent scoreboard, the entry-to-exit pipeline, and every trading surface and doc in one place.
...
1. Load http://localhost:3000/trading. Record the HTTP status, every console
...
- [ ] Local load of /trading: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-trading.md
