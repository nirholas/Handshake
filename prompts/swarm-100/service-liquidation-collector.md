# Service audit: services/liquidation-collector
...
1. Read services/liquidation-collector/README.md and its run configuration.
2. Determine where it runs (Cloud Run service, sidecar, local daemon) and
   whether that instance is alive right now; the CLAUDE.md playbook has the
   gcloud commands (remember: export PATH="$HOME/google-cloud-sdk/bin:$PATH").

## Task: bring this service to verified-healthy

1. README accuracy against the code; fix stale claims.
2. Build and start it locally per its own instructions; exercise its core
   path for real.
3. Tests: run what exists, add a core-path smoke test if none exists.
4. If it has a live instance: recent logs free of unhandled errors, or root
   causes fixed and committed (deploys stay owner-gated).

## Definition of done

- [ ] Local build and a real core-path exercise both succeed.
- [ ] Tests pass, including a core-path smoke test.
- [ ] README verified accurate.
- [ ] Live-instance health verified or fixes committed and flagged.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/service-liquidation-collector.md
