# Route audit: /forgot-password (Forgot password)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/forgot-password')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/forgot-password" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/forgot-password
...
Route: /forgot-password ("Forgot password", section "account").
Declared purpose: Request a password-reset email.
...
1. Load http://localhost:3000/forgot-password. Record the HTTP status, every console
...
- [ ] Local load of /forgot-password: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-forgot-password.md
