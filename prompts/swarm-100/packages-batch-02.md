# Package audit batch 02: agent-sniper, agentcore-payments-mcp, agora-mcp, alerts-mcp, alibaba-cloud-mcp, assistant-mcp
...
For each package below: read its package.json and README.md, then find its
tests (its own directory and tests/ at the repo root).

## Task: audit and fix these packages

Packages in this batch:

- packages/agent-sniper
- packages/agentcore-payments-mcp
- packages/agora-mcp
- packages/alerts-mcp
- packages/alibaba-cloud-mcp
- packages/assistant-mcp

For each one:

1. README: present (repo standard is 100% coverage under packages/) and
   accurate: what it does, install and use, public exports, one runnable
   example. Run the example for real.
2. Exports resolve: import the package's declared entry points with node and
   confirm they load without error.
3. Tests: run the tests that cover it; if its core path has none, add a real
   one.
4. Hard-rule scan across its source: no stubs, no mock fallbacks, no
   unfinished markers, no commented-out code. Fix at root.
5. package.json sanity: name, version, license, exports and files fields
   consistent with what is actually shipped.

## Definition of done

- [ ] Every package in the batch: README verified accurate, example executed,
      entry points imported cleanly, tests green with core-path coverage.
- [ ] Zero hard-rule violations remain in these packages.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/packages-batch-02.md
