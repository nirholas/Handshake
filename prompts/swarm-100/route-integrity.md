# Route audit: /integrity (Custody Integrity)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/integrity')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/integrity" in vercel.json (the live
...
Route: /integrity ("Custody Integrity", section "crypto").
Declared purpose: Provable, on-chain-anchored custody for every three.ws agent wallet. The platform commits a Merkle root over every wallet's on-chain state to Solana: verify the latest root yourself, no account required.
...
1. Load http://localhost:3000/integrity. Record the HTTP status, every console
...
- [ ] Local load of /integrity: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-integrity.md
