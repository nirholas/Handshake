# Roadmap build: Phase 2: week-2 retention measurement for minted agents
...
The phase 2 verification metric is week-2 retention on minted agents. Build
the measurement so the number exists.

Step 0: read the existing analytics and usage-event plumbing (flush-usage-events
cron, dashboard analytics surfaces) and check what return-visit signal is
already captured.

Build: privacy-respecting return-visit tracking tied to agent ownership (no
third-party analytics; use the platform's own event store), a weekly rollup
(new cron following the existing cron patterns, registered in vercel.json so
scheduler sync picks it up), and a retention chart on the internal analytics
dashboard. Convert relative dates to absolute in stored rollups.
...
- [ ] Events flow end to end: a real return visit lands in the store and the
      rollup computes a cohort number.
- [ ] Cron registered in vercel.json with clean syntax and drift checks.
- [ ] Retention chart live on the analytics dashboard with real data.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p2-retention-instrumentation.md
