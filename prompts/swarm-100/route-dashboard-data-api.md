# Route audit: /dashboard/data-api (Dashboard · Data API)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/dashboard/data-api')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/dashboard/data-api" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/dashboard/data-api
...
Route: /dashboard/data-api ("Dashboard · Data API", section "account").
Declared purpose: Developer console for the three.ws Data API: buy the Premium pass with $THREE, SOL, or USDC, manage API keys, watch usage, and copy quickstarts.
...
1. Load http://localhost:3000/dashboard/data-api. Record the HTTP status, every console
...
- [ ] Local load of /dashboard/data-api: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-dashboard-data-api.md
