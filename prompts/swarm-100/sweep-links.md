# Sweep: Zero broken links, zero stub hrefs
...
Run the link audit. Broken internal links get fixed or their referents restored. Stub hrefs (href="#", javascript:void(0)) get triaged: a real control with a JS handler becomes a proper button with ARIA and keyboard support; a genuinely dead path gets wired to the surface it promises or removed. Re-run npm run audit:a11y afterwards since the markup changes.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-links.md
