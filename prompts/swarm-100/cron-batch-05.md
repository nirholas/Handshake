# Cron audit batch 05: /api/cron/irl-reap and 7 more
...
2. Scheduler truth (crons run on Cloud Scheduler, never in-process):
       ...
       gcloud scheduler jobs list --project aerial-vehicle-466722-p5 --location us-central1 | grep -E 'irl-reap|settlement-verify|expire-pending-purchases|confirm-pending-purchases|uptime-check|world-health|forge-smoke|forge-finalize'
...
- /api/cron/irl-reap
- /api/cron/settlement-verify
- /api/cron/expire-pending-purchases
- /api/cron/confirm-pending-purchases
- /api/cron/uptime-check
- /api/cron/world-health
- /api/cron/forge-smoke
- /api/cron/forge-finalize
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/cron-batch-05.md
