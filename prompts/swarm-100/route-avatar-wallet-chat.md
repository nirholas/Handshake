# Route audit: /avatar-wallet-chat (Avatar Wallet)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/avatar-wallet-chat')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/avatar-wallet-chat" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/avatar-wallet-chat
...
Route: /avatar-wallet-chat ("Avatar Wallet", section "crypto").
Declared purpose: An embeddable 3D AI avatar that holds a Solana wallet, chats, and can autonomously send SOL.
...
1. Load http://localhost:3000/avatar-wallet-chat. Record the HTTP status, every console
...
- [ ] Local load of /avatar-wallet-chat: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-avatar-wallet-chat.md
