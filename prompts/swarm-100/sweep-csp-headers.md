# Sweep: CSP and security headers verified
...
Run the CSP audit and fix findings. Then verify the security headers production actually serves (curl -sI https://three.ws and key subpages): CSP, HSTS, X-Content-Type-Options, and frame policy must match what the repo intends to serve. Fix at the server or config source, commit, and flag if a deploy is needed to take effect.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-csp-headers.md
