# Cron audit batch 04: /api/cron/kol-tracker-refresh and 7 more
...
2. Scheduler truth (crons run on Cloud Scheduler, never in-process):
       ...
       gcloud scheduler jobs list --project aerial-vehicle-466722-p5 --location us-central1 | grep -E 'kol-tracker-refresh|quota-check|x402-directory-registrar|relayer-balance-check|run-buyback|run-distribute-payments|run-three-buyback|audit-log-cleanup'
...
- /api/cron/kol-tracker-refresh
- /api/cron/quota-check
- /api/cron/x402-directory-registrar
- /api/cron/relayer-balance-check
- /api/cron/run-buyback
- /api/cron/run-distribute-payments
- /api/cron/run-three-buyback
- /api/cron/audit-log-cleanup
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/cron-batch-04.md
