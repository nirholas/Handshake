# Route audit: /animations (Animation Gallery)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/animations')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/animations" in vercel.json (the live
...
Route: /animations ("Animation Gallery", section "main").
Declared purpose: Browse and preview public animations authored by the three.ws community. Apply any clip to your avatar in the Animation Studio.
...
1. Load http://localhost:3000/animations. Record the HTTP status, every console
...
- [ ] Local load of /animations: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-animations.md
