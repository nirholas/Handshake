# Route audit: /avatar-sdk (Avatar SDK)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/avatar-sdk')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/avatar-sdk" in vercel.json (the live
...
Route: /avatar-sdk ("Avatar SDK", section "build").
Declared purpose: The @three-ws/avatar package: embed the platform 3D avatars in any site with a few lines of JavaScript.
...
1. Load http://localhost:3000/avatar-sdk. Record the HTTP status, every console
...
- [ ] Local load of /avatar-sdk: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-avatar-sdk.md
