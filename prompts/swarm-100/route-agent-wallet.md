# Route audit: /agent-wallet (Agent Wallet)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agent-wallet')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agent-wallet" in vercel.json (the live
...
Route: /agent-wallet ("Agent Wallet", section "crypto").
Declared purpose: The wallet surface for your agents: balances, funding, and activity.
...
1. Load http://localhost:3000/agent-wallet. Record the HTTP status, every console
...
- [ ] Local load of /agent-wallet: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agent-wallet.md
