...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/voice')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/voice" in vercel.json (the live
...
Route: /voice ("Voice Lab", section "build").
Declared purpose: Browse about 850 voices across Microsoft Edge, Gemini, NVIDIA, OpenAI and ElevenLabs, preview any of them, or clone your own from a short recording, then give it to your agent.
...
1. Load http://localhost:3000/voice. Record the HTTP status, every console
...
- [ ] Local load of /voice: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-voice.md
