# Route audit: /autopilot (Coin Autopilot)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/autopilot')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/autopilot" in vercel.json (the live
...
Route: /autopilot ("Coin Autopilot", section "main").
Declared purpose: A hands-off token cockpit: set the rules: buys, sells, and guardrails: and let your agent run the coin autonomously.
...
1. Load http://localhost:3000/autopilot. Record the HTTP status, every console
...
- [ ] Local load of /autopilot: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-autopilot.md
