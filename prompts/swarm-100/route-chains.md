# Route audit: /chains (Blockchain TVL by Chain)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/chains')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/chains" in vercel.json (the live
...
Route: /chains ("Blockchain TVL by Chain", section "crypto").
Declared purpose: A leaderboard of every blockchain by total value locked, with each chain's dominance share of on-chain DeFi. Live from DeFiLlama: see where liquidity actually sits across L1s and L2s.
...
1. Load http://localhost:3000/chains. Record the HTTP status, every console
...
- [ ] Local load of /chains: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-chains.md
