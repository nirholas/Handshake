# Roadmap build: Phase 3: subscriptions and DCA finished as a product surface
...
Roadmap: recurring on-chain payments to creators, with cron infra in place.
The crons run-subscriptions and run-dca already exist; finish the product
around them.

Step 0: read both cron handlers and every surface that references
subscriptions or DCA; run each handler's validation path locally and measure
what a user can actually set up today versus what silently no-ops.

Build: complete the user-facing lifecycle: create, view, pause, resume, and
cancel a recurring payment or DCA schedule, with designed states, real
schedule execution through the existing crons, and a creator-side view of
incoming subscriptions. Proof runs use the test lane; anything moving real
funds stays gated.
...
- [ ] Full lifecycle exercised end to end on the test lane, including a cron
      tick that executes a due schedule.
- [ ] Creator-side incoming view live with real schedule data.
- [ ] Handler edge cases (insufficient balance, revoked auth) produce
      designed, recorded outcomes; tests cover them.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p3-subscriptions-dca.md
