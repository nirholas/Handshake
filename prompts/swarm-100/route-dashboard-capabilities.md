# Route audit: /dashboard/capabilities (Capabilities: Alpha Hunt, Launcher, Auto-Claim & Market Maker)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/dashboard/capabilities')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/dashboard/capabilities" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/dashboard/capabilities
...
Route: /dashboard/capabilities ("Capabilities: Alpha Hunt, Launcher, Auto-Claim & Market Maker", section "crypto").
Declared purpose: Live command center for all 4 autonomous agent capabilities: Alpha Hunt smart-money scoring, autonomous Coin Launcher on schedule, Creator Auto-Claim for fee harvesting, and Market Maker with Jito execution.
...
1. Load http://localhost:3000/dashboard/capabilities. Record the HTTP status, every console
...
- [ ] Local load of /dashboard/capabilities: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-dashboard-capabilities.md
