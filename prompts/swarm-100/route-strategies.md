# Route audit: /strategies (Strategy Objects)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/strategies')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/strategies" in vercel.json (the live
...
Route: /strategies ("Strategy Objects", section "crypto").
Declared purpose: A marketplace of how agents trade: ownable, forkable, leaderboard-ranked trade strategies your agent can equip and run on-chain within your spend policy.
...
1. Load http://localhost:3000/strategies. Record the HTTP status, every console
...
- [ ] Local load of /strategies: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-strategies.md
