# Sweep: Unknown routes land on a designed 404
...
memory of what used to fail:
    ...
    curl -so /dev/null -w "%{http_code}\n" https://three.ws/definitely-not-a-real-page-xyz
...
Verify locally and in production that an unknown route renders the designed 404 page with navigation back into the product, correct 404 status, and zero console errors. Same check for a trailing-slash variant, an unknown /api/ path (JSON 404, not HTML), and an unknown /docs/ path. Fix the server route table or 404 page at root.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-404-handling.md
