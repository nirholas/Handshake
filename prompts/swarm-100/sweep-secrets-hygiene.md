# Sweep: No secrets in tracked files
...
Sweep every tracked file for credential material: private keys, API keys, bearer tokens, connection strings (npx gitleaks detect --no-git -v or equivalent patterns via grep). .env files must be untracked. Anything found gets removed from the tree, the affected credential named in the report for rotation (rotation itself may be owner-gated), and a guard added so the same class cannot land again.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-secrets-hygiene.md
