# Route audit: /brain (Multi-LLM Brain)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/brain')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/brain" in vercel.json (the live
...
Route: /brain ("Multi-LLM Brain", section "labs").
Declared purpose: Send one prompt to Claude, GPT, Qwen, ModelScope, and Groq simultaneously. Side-by-side streaming with latency and token stats.
...
1. Load http://localhost:3000/brain. Record the HTTP status, every console
...
- [ ] Local load of /brain: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-brain.md
