# Docs audit batch 41: /tutorials/emotion-from-data through /tutorials/self-host-agent-backend
...
3. Probe production for each path:
       ...
       for p in /tutorials/emotion-from-data /tutorials/custom-skill /tutorials/register-onchain /tutorials/mint-pumpfun-token /tutorials/paid-x402-endpoint /tutorials/monetize-mcp-server /tutorials/x402-server-sdk /tutorials/self-host-agent-backend; do printf '%s ' "$p"; curl -so /dev/null -w '%{http_code}\n' "https://three.ws$p"; done

## Task: audit this batch of docs pages, then fix everything found

Pages in this batch:

- /tutorials/emotion-from-data ("Tutorial · Animations from data: wire your agent's gestures to real APIs and AI")
- /tutorials/custom-skill ("Tutorial · Build a custom skill")
- /tutorials/register-onchain ("Tutorial · Register your agent on-chain (ERC-8004)")
- /tutorials/mint-pumpfun-token ("Tutorial · Mint an agent token on Pump.fun")
- /tutorials/paid-x402-endpoint ("Tutorial · Build a paid x402 endpoint your agent calls")
- /tutorials/monetize-mcp-server ("Tutorial · Monetize an MCP server: charge per tool call")
- /tutorials/x402-server-sdk ("Tutorial · Meter any API with the x402-server SDK")
- /tutorials/self-host-agent-backend ("Tutorial · Self-host the agent backend")
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/docs-batch-41.md
