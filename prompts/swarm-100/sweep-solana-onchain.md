# Sweep: Solana and on-chain verifications green
...
memory of what used to fail:
    ...
    npm run verify:solana && npm run verify:onchain && npm run smoke:onchain
...
Run all three. Fix RPC config, drifted addresses, and broken read paths at root. Read-only: no transaction may be signed or sent. Solana is the home chain; if an EVM leg is broken, fix it too but never at the cost of the Solana path.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-solana-onchain.md
