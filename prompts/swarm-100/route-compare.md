# Route audit: /compare (Compare Crypto Side by Side)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/compare')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/compare" in vercel.json (the live
...
Route: /compare ("Compare Crypto Side by Side", section "crypto").
Declared purpose: Compare cryptocurrencies head to head: overlay normalized price performance and line up market cap, volume, FDV, supply, and all-time-high stats for up to four coins at once. The selection is saved to the URL so any matchup is shareable.
...
1. Load http://localhost:3000/compare. Record the HTTP status, every console
...
- [ ] Local load of /compare: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-compare.md
