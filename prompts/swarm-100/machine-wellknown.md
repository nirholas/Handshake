# Machine surface audit: .well-known and MCP discovery endpoints
...
Fetch each endpoint from production and record status plus content type:

    curl -si https://three.ws/api/mcp-policy | head -20
    curl -si https://three.ws/.well-known/x402 | head -20
    curl -si https://three.ws/.well-known/agent-attestation-schemas | head -20
    curl -si https://three.ws/.well-known/oauth-authorization-server | head -20
    curl -si https://three.ws/.well-known/chat-plugin.json | head -20
    curl -si https://three.ws/api/mcp | head -20
    curl -si https://three.ws/.well-known/mcp.json | head -20
    curl -si https://three.ws/.well-known/agent-card.json | head -20
    curl -si https://three.ws/.well-known/x402.json | head -20

## Task: verify every machine-readable discovery endpoint

Endpoints: /api/mcp-policy, /.well-known/x402, /.well-known/agent-attestation-schemas, /.well-known/oauth-authorization-server, /.well-known/chat-plugin.json, /api/mcp, /.well-known/mcp.json, /.well-known/agent-card.json, /.well-known/x402.json

1. Each JSON endpoint parses as strict JSON; each declared field points at a
   live capability (URLs it names must respond; tools it lists must exist in
   the MCP catalog; payment schemes it advertises must match the x402 code).
2. Cross-consistency: agent-card, mcp.json, x402 descriptors, and the OAuth
   metadata must not contradict each other or the code in api/ that serves
   them.
3. Same checks locally against a started server so a fix is verifiable before
   any deploy.
4. Fix every mismatch at the source that generates or serves the payload.

## Definition of done

- [ ] Every endpoint above returns 200 with valid, strictly-parsed content.
- [ ] Every URL, tool name, and scheme these payloads advertise verified live.
- [ ] Zero contradictions across the discovery surfaces.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/machine-wellknown.md
