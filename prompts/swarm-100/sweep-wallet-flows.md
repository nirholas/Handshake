# Sweep: Wallet flow audits clean
...
memory of what used to fail:
    ...
    npm run audit:wallet-flows && npm run audit:service-wallets
...
Run both audits. These are read-only checks of wallet plumbing and service wallet health; fix code findings at root. Anything that would move funds is out of scope here and stays behind the CLAUDE.md gate.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-wallet-flows.md
