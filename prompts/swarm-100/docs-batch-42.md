# Docs audit batch 42: /tutorials/multi-agent-coordination through /docs/rig-doctor
...
3. Probe production for each path:
       ...
       for p in /tutorials/multi-agent-coordination /tutorials/mcp-server-for-your-agent /tutorials/skill-with-database-auth /tutorials/deploy-to-vercel-custom-domain /tutorials/claim-threews-name /tutorials/mine-vanity-address /docs/avatar-cli /docs/rig-doctor; do printf '%s ' "$p"; curl -so /dev/null -w '%{http_code}\n' "https://three.ws$p"; done

## Task: audit this batch of docs pages, then fix everything found

Pages in this batch:

- /tutorials/multi-agent-coordination ("Tutorial · Multi-agent coordination")
- /tutorials/mcp-server-for-your-agent ("Tutorial · Expose your agent as an MCP server")
- /tutorials/skill-with-database-auth ("Tutorial · Custom skill with database + auth")
- /tutorials/deploy-to-vercel-custom-domain ("Tutorial · Deploy to Vercel with a custom domain")
- /tutorials/claim-threews-name ("Tutorial · Claim a .threews.sol name & get paid by name")
- /tutorials/mine-vanity-address ("Tutorial · Mine a Solana vanity address")
- /docs/avatar-cli ("Docs: Avatar CLI")
- /docs/rig-doctor ("Rig Doctor reference")
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/docs-batch-42.md
