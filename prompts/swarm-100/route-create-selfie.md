# Route audit: /create/selfie (Selfie to Avatar)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/create/selfie')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/create/selfie" in vercel.json (the live
...
Route: /create/selfie ("Selfie to Avatar", section "build").
Declared purpose: One selfie. About a minute. A rigged 3D avatar that works everywhere.
...
1. Load http://localhost:3000/create/selfie. Record the HTTP status, every console
...
- [ ] Local load of /create/selfie: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-create-selfie.md
