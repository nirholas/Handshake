# Route audit: /agent-screen (Agent Screen)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agent-screen')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agent-screen" in vercel.json (the live
...
Route: /agent-screen ("Agent Screen", section "build").
Declared purpose: Watch your agent work in real time: split view with the agent's live screen on the left and their 3D avatar rendered as a webcam on the right.
...
1. Load http://localhost:3000/agent-screen. Record the HTTP status, every console
...
- [ ] Local load of /agent-screen: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agent-screen.md
