# Cron audit batch 06: /api/llm/health and 7 more
...
2. Scheduler truth (crons run on Cloud Scheduler, never in-process):
       ...
       gcloud scheduler jobs list --project aerial-vehicle-466722-p5 --location us-central1 | grep -E 'health|cleanup-csrf-tokens|treasury-autopilot|process-withdrawals|erc8004-crawl|solana-autodeploy|solana-attestations-crawl|trader-score-attest'
...
- /api/llm/health
- /api/cron/cleanup-csrf-tokens
- /api/cron/treasury-autopilot
- /api/cron/process-withdrawals
- /api/cron/erc8004-crawl
- /api/cron/solana-autodeploy
- /api/cron/solana-attestations-crawl
- /api/cron/trader-score-attest
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/cron-batch-06.md
