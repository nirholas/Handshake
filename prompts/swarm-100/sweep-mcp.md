...
memory of what used to fail:
    ...
    npm run audit:mcp && npm run smoke:mcp && npm run test:mcp && npm run audit:mcp-golden && npm run audit:mcp-safety && npm run audit:mcp-catalog
...
Run all six. Golden drift means a tool description changed: regenerate via the documented update flow only if the change is intended, otherwise fix the tool. Safety findings get fixed in the tool, never waived.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-mcp.md
