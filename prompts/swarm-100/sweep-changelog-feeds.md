# Sweep: Changelog pipeline healthy
...
memory of what used to fail:
    ...
    npm run build:pages && npm run changelog:gaps
...
Regenerate the feeds and run the gap check. Malformed entries get fixed. Every user-visible change in recent history that lacks a holder-readable changelog entry gets one (plain language, correct tags, live link paths). Never run the manual push scripts; delivery is the cron's job.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-changelog-feeds.md
