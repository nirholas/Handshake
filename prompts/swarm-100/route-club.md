...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/club')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/club" in vercel.json (the live
...
Route: /club ("Pole Club", section "crypto").
Declared purpose: A 3D club where dancers only perform when you send a micro-tip. Pay $0.001 per routine, settled on-chain via x402 on Solana.
...
1. Load http://localhost:3000/club. Record the HTTP status, every console
...
- [ ] Local load of /club: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-club.md
