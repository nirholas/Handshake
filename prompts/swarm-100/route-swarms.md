# Route audit: /swarms (Trading Swarms)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/swarms')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/swarms" in vercel.json (the live
...
Route: /swarms ("Trading Swarms", section "crypto").
Declared purpose: Coordinated groups of trading agents working a shared strategy, with live positions and performance.
...
1. Load http://localhost:3000/swarms. Record the HTTP status, every console
...
- [ ] Local load of /swarms: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-swarms.md
