# Sweep: Repository guards proven
...
memory of what used to fail:
    ...
    npm run audit:guards && npm run build:guards
...
Run both. Every guard must still fire on its proof case and every guard doc must match its implementation. A guard that silently stopped working is a rule that silently stopped existing.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-guards.md
