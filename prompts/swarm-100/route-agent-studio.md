# Route audit: /agent-studio (Agent Studio)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agent-studio')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agent-studio" in vercel.json (the live
...
Route: /agent-studio ("Agent Studio", section "build").
Declared purpose: Author your agent's brain, memory, body, money, and skills in one place, with a live 3D avatar that updates everywhere as you edit.
...
1. Load http://localhost:3000/agent-studio. Record the HTTP status, every console
...
- [ ] Local load of /agent-studio: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agent-studio.md
