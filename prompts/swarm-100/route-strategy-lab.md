# Route audit: /strategy-lab (Strategy Lab)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/strategy-lab')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/strategy-lab" in vercel.json (the live
...
Route: /strategy-lab ("Strategy Lab", section "crypto").
Declared purpose: Build DCA and subscription strategies that trade on-chain on your behalf.
...
1. Load http://localhost:3000/strategy-lab. Record the HTTP status, every console
...
- [ ] Local load of /strategy-lab: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-strategy-lab.md
