# Route audit: /skills (Skills Marketplace)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/skills')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/skills" in vercel.json (the live
...
Route: /skills ("Skills Marketplace", section "main").
Declared purpose: Browse, search, and install agent skills: tool packs, knowledge bases, and capabilities that make AI agents smarter. Now lives inside the Marketplace; this path forwards there.
...
1. Load http://localhost:3000/skills. Record the HTTP status, every console
...
- [ ] Local load of /skills: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-skills.md
