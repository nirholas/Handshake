# Route audit: /walk-leaderboard (Walk Leaderboard)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/walk-leaderboard')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/walk-leaderboard" in vercel.json (the live
...
3. Production right now:
       ...
       curl -so /dev/null -w '%{http_code}\n' https://three.ws/walk-leaderboard
...
Route: /walk-leaderboard ("Walk Leaderboard", section "labs").
Declared purpose: Global leaderboard for the walking avatar: distance walked, sites visited, and time, ranked daily, weekly, and all-time.
...
1. Load http://localhost:3000/walk-leaderboard. Record the HTTP status, every console
...
- [ ] Local load of /walk-leaderboard: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-walk-leaderboard.md
