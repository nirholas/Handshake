# Route audit: /vanity/verify (Verify a Vanity Receipt)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/vanity/verify')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/vanity/verify" in vercel.json (the live
...
Route: /vanity/verify ("Verify a Vanity Receipt", section "crypto").
Declared purpose: Independently verify a three.ws provably-fair vanity wallet receipt: entirely in your browser. Prove the key was generated fresh under a committed seed and the operator never kept a copy.
...
1. Load http://localhost:3000/vanity/verify. Record the HTTP status, every console
...
- [ ] Local load of /vanity/verify: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-vanity-verify.md
