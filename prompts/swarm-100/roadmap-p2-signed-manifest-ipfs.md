# Roadmap build: Phase 2: signed per-agent system prompt pinned to IPFS
...
Roadmap: the per-agent system prompt stored in the manifest, signed, and
pinned to IPFS so an agent's behavior is verifiable and portable.

Step 0: read specs/ for the manifest spec and the existing provenance and
signing code (grep anchor_provenance and manifest signing across api/ and
packages/provenance-mcp); check what pinning infrastructure already exists
in the repo before adding any.

Build: on persona save, produce a canonical manifest document, sign it with
the platform's existing signing identity, pin it (reuse existing pinning
plumbing if present; otherwise select a maintained OSS pinning client and
wire it behind its env var), store the CID on the agent record, and expose
verification: anyone with the CID can check the signature and diff the
manifest against live agent config.
...
- [ ] A real agent's manifest signed, pinned, CID stored, and independently
      verified by fetching the CID and checking the signature.
- [ ] Manifest spec in specs/ updated to cover the signed envelope.
- [ ] Verification path covered by a test; npm test green; check:rules clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p2-signed-manifest-ipfs.md
