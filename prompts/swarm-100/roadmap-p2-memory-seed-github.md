# Roadmap build: Phase 2: agent memory seeding from GitHub with explicit consent
...
Same deliverable as the X seeding order, for GitHub, and independent of it:
build to the same memory interfaces whether or not any other seeding source
exists.

Step 0: read packages/agent-memory and the api memory handlers; check for
any existing GitHub OAuth plumbing in api/auth before adding one.

Build: consent-first GitHub connect (OAuth), ingest of public profile,
pinned repos, and READMEs the user selects, seeding the agent's memory so
the agent can speak concretely about the user's work. Revocation deletes
seeded memories.
...
- [ ] Connect, consent, seed, converse, revoke, verify-deleted exercised end
      to end against a real GitHub account.
- [ ] User selects what is ingested; nothing outside the selection is stored.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p2-memory-seed-github.md
