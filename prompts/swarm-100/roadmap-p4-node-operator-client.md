# Roadmap build: Phase 4: node operator client
...
Roadmap: anyone can run an inference node. Build the operator client:
Docker plus GPU drivers, registration, job execution.

Step 0: read the phase 4 protocol spec in specs/ if it has landed; if it has
not, derive the wire contract from the platform's existing worker job
format and state that choice in one line of the report (this order never
blocks on another).

Build: a self-contained operator client (its own top-level directory with a
README per the repo standard): Dockerfile, config for keys and endpoints,
registration call against the platform, a job loop that executes real
inference with a small open model as the built-in proof workload, and
signed responses. Prove it end to end locally: register, receive a job,
...
- [ ] Fresh-clone build of the client succeeds; end-to-end local run
      completes register, job, signed result; transcript in the report.
- [ ] README covers install, config, run, and verify for a zero-context
      operator.
- [ ] npm test green for the client's test suite; check:rules clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p4-node-operator-client.md
