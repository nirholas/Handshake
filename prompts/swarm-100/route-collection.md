# Route audit: /collection (My Collection)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/collection')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/collection" in vercel.json (the live
...
Route: /collection ("My Collection", section "main").
Declared purpose: View all the skills and subscriptions you've unlocked on three.ws.
...
1. Load http://localhost:3000/collection. Record the HTTP status, every console
...
- [ ] Local load of /collection: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-collection.md
