# Route audit: /categories (Crypto Categories by Market Cap)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/categories')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/categories" in vercel.json (the live
...
Route: /categories ("Crypto Categories by Market Cap", section "crypto").
Declared purpose: Every crypto sector ranked by market cap: smart contract platforms, AI agents, memecoins, DeFi, gaming, and more: with 24h change, volume, and the top coins in each. See which narratives are leading the market right now.
...
1. Load http://localhost:3000/categories. Record the HTTP status, every console
...
- [ ] Local load of /categories: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-categories.md
