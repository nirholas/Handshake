# Sweep: MCP server manifests consistent
...
Validate every server*.json manifest at the repo root plus .well-known/mcp.json: strict JSON, every declared tool exists in the served catalog, every endpoint URL responds, and registry listings (Anthropic MCP registry, glama.json) describe tools that actually ship. Fix drift at the manifest or the tool, whichever is wrong.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/sweep-mcp-manifests.md
