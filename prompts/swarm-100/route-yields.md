# Route audit: /yields (DeFi Yields Explorer)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/yields')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/yields" in vercel.json (the live
...
Route: /yields ("DeFi Yields Explorer", section "crypto").
Declared purpose: Explore roughly 15,000 live DeFi yield pools across every chain and protocol: filter by chain, project, stablecoin exposure, and minimum TVL, sort by APY or TVL, and open any pool to see its full APY and TVL history. Live from DeFiLlama, no key required.
...
1. Load http://localhost:3000/yields. Record the HTTP status, every console
...
- [ ] Local load of /yields: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-yields.md
