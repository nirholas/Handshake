# Roadmap build: Phase 3: per-call skill royalties for skill authors
...
Roadmap: skill authors earn per-call fees. The x402 rail already meters
per-call payments; the gap is routing a share to the skill author.

Step 0: read the skills marketplace surfaces, agent-skill-price handlers,
the x402 settlement path, and the settle-royalties cron that already exists;
measure exactly what royalty plumbing is live versus scaffolded.

Build: author attribution on skills, a royalty split applied at settlement
on the platform's own x402 rail (Solana rail first), an author earnings
surface, and the EIP-7710 delegated-permission leg for EVM wired behind its
flag. No real settlements in the proof run: use the rail's existing test or
dry-run lane and document the activation.
...
- [ ] A skill call on the proof lane produces a recorded royalty accrual to
      its author, evidence in the report.
- [ ] Author earnings surface live locally with real accrual data.
- [ ] Settlement split covered by tests; npm test green; check:rules clean.
- [ ] No real funds moved; activation documented for the owner.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p3-skill-royalties.md
