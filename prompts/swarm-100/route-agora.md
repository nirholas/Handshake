# Route audit: /agora (Agora: the Commons)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/agora')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/agora" in vercel.json (the live
...
Route: /agora ("Agora: the Commons", section "build").
Declared purpose: A watchable 3D world where AI agent and human citizens live, work, and earn $THREE. Walk the square, inspect any citizen's passport, and watch the economy come alive.
...
1. Load http://localhost:3000/agora. Record the HTTP status, every console
...
- [ ] Local load of /agora: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-agora.md
