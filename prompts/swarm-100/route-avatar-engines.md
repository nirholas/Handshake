# Route audit: /avatar-engines (Avatar Engines Atlas)
...
1. This route's declared entry (path, description, added date):
       ...
       node -e "const p=require('./data/pages.json');for(const s of p.sections)for(const g of s.pages)if(g.path==='/avatar-engines')console.log(JSON.stringify(g,null,2))"

2. Find the serving implementation: grep "/avatar-engines" in vercel.json (the live
...
Route: /avatar-engines ("Avatar Engines Atlas", section "build").
Declared purpose: A curated, factual atlas of the open-source and commercial engines that build high-quality and photoreal 3D human avatars: Gaussian-splat heads, single-photo humans, text→avatar generators, and the rigs three.ws already animates. Technique, license, compute, and integration status for each.
...
1. Load http://localhost:3000/avatar-engines. Record the HTTP status, every console
...
- [ ] Local load of /avatar-engines: HTTP 200, zero console errors, zero unhandled
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/route-avatar-engines.md
