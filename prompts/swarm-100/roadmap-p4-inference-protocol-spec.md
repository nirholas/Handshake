# Roadmap build: Phase 4: open inference protocol spec plus reference implementation
...
Roadmap: an open protocol for agent inference (model weights, GPU runtime,
signed responses) so inference decouples from any single provider.

Step 0: read the existing worker protocol (how the platform's own GPU
workers receive jobs and return results), the LLM failover chain docs, and
specs/ conventions.

Build: the protocol spec in specs/ (job envelope, capability advertisement,
signed response format, verification rules), written to the repo's spec
standard, plus a reference implementation: one of the platform's existing
workers speaking the protocol behind a flag, and a verifier module that
checks response signatures. The spec is the deliverable other orders build
against, but this order stands alone: spec plus reference prove each other.
...
- [ ] Spec landed in specs/ covering envelope, advertisement, signing, and
      verification, with no unresolved sections.
- [ ] Reference worker serves one real job through the protocol locally;
      verifier validates the signed response; transcript in the report.
- [ ] Verifier covered by tests; npm test green; check:rules clean.
...
3. Delete this prompt file in that same commit:
       ...
       git rm prompts/swarm-100/roadmap-p4-inference-protocol-spec.md
