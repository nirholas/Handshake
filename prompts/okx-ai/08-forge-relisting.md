...
`/workspaces/three.ws`, or say "execute `prompts/okx-ai/08-forge-relisting.md`". Read
`prompts/okx-ai/00-CONTEXT.md`, the 2026-08-22 entry in `prompts/okx-ai/PROGRESS.md`,
`prompts/okx-ai/RUNBOOK.md` and `CLAUDE.md` first.

This supersedes `05-relisting-resubmission.md`, which submits the retired 11-row catalog.
Do not run 05.
...
Session preflight per `00-CONTEXT.md`. Follow the `okx-agent-identity` update flow exactly:
...
Append a dated entry to `prompts/okx-ai/PROGRESS.md`: the delta that was submitted, the CLI
...
Funding. A real settled payment is a separate gate (`04-e2e-real-payment-test.md`) and is
