...
| The database | `DATABASE_URL` in `.env` (Neon); production's authoritative copy is on the Cloud Run service env. Migrations: `npm run db:migrate` (dry-run) / `-- --apply`. `forge_creations` carries per-generation backend/status/error/prompt — the fastest ground truth for generation issues. |
...
| How to deploy | API/frontend, from a CLEAN worktree (`git worktree add --detach /workspaces/.deploy-wt HEAD`, hardlink THREE artifacts with `cp -al` on the same filesystem, not just the first — `node_modules`, `chat/node_modules`, and `character-studio/build`, plus copy `.env`; omitting either nested one kills the build minutes in, with an OOM exit 144 on the avatar-studio rebuild or `Cannot find package '@sveltejs/vite-plugin-svelte'` on the chat build, see `docs/ops/gcp-production.md`): run `npm run build:gcp` then `gcloud builds submit --config server/cloudbuild.yaml --region us-central1 --project aerial-vehicle-466722-p5`, then `npm run deploy:gcp:purge-cdn` (or the whole build+submit+purge in one: `npm run deploy:gcp:full`). ORDER IS LOAD-BEARING and `build:gcp` encodes it: the frontend `vite build` WIPES `dist/`, so it must run FIRST, then the UMD lib build, then `publish:lib` (which mirrors the lib into `dist/`), then `build:info`, then `check:dist`, then `check:pages`. Do NOT hand-run `build:vercel` as the frontend build — it builds the SDK/lib sub-artifacts, NOT the static HTML pages, so `dist/` ends up without `/`, `/create`, etc. and `check:dist` fails. `check:dist` needs the UMD bundle (publish:lib) present. GPU workers: their own `workers/<name>/cloudbuild.yaml`. EVERY cloudbuild config must pin `serviceAccount: …/three-ws-build@…` (default compute SA was deleted) and manual submits need `--substitutions=SHORT_SHA=manual$(date +%s)` when the config tags images with `$SHORT_SHA`. Verify the deploy landed: `curl -s https://three.ws/api/version` returns the live commit SHA + Cloud Run revision, and `npm run smoke:prod` sweeps every page declared in `data/pages.json` against the live site (`deploy:gcp` runs it automatically after the purge). The CDN purge is synchronous on purpose — do not re-add `--async`, or post-deploy checks read stale edge content and report phantom failures. |

The ONLY standing stop-and-ask gates (everything else: proceed, then report):
1. Irreversible on-chain / spend actions (the gate below).
...
- **Delivery to holders is automatic.** `/api/cron/changelog-push` (Cloud Scheduler, every 20 min) reads the feed baked into the running image, diffs it against DB state (`app_settings`), and posts anything new to the holders' Telegram channel (@three_ws) and the @trythreews X release thread. An entry goes out on its own shortly after the deploy that ships it; there is no manual push step. Credentials live on the Cloud Run service. Do NOT run `npm run changelog:push` / `changelog:push:x` for routine releases anymore: their file-based state (`data/changelog-*-state.json`) is separate from the cron's DB state, so a manual push double-posts. The scripts remain for `--dry-run` previews and owner-directed backfills only.
