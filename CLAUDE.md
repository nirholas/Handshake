...
2. **Build: `npm run build:gcp`.** ORDER IS LOAD-BEARING and `build:gcp` encodes it: the frontend `vite build` WIPES `dist/`, so it must run FIRST, then the UMD lib build, then `publish:lib` (which mirrors the lib into `dist/`), then `build:info`, then `check:dist`, then `check:pages`. Do NOT hand-run `build:vercel` as the frontend build: it builds the SDK/lib sub-artifacts, NOT the static HTML pages, so `dist/` ends up without `/`, `/create`, etc. and `check:dist` fails. `check:dist` needs the UMD bundle (`publish:lib`) present.
...
- **Production runs on Google Cloud Run, NOT Vercel** (migrated 2026-07-07 after Vercel disabled the deployment). One container ([server/index.mjs](server/index.mjs)) serves the static frontend, the vercel.json route table, and all `api/**` handlers; the crons (89 as of 2026-07-18, see vercel.json) run on Cloud Scheduler. `vercel.json` is a LIVE config file consumed by the server (routes + crons): never delete it as a leftover. GCP builds/deploys must pin the `three-ws-build@` (build) and `three-ws@` (runtime) service accounts; the project's default compute SA was deleted. Deploys: see the "Deploy runbook" section above.
