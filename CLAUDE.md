...
2. **Build: `npm run build:gcp`.** ORDER IS LOAD-BEARING and `build:gcp` already encodes it. Never hand-run the steps in a different order. The full chain, in order:
   ...
   `check:conflicts` → `ensure:avatar-studio` → `build:info:snapshot` → `build:chat` → `build` (the frontend `vite build`) → `build:lib:full` (the UMD lib) → `publish:lib` → `build:info` → `check:dist` → `check:pages`
