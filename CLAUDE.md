...
- After the change is deployed, `npm run changelog:push` posts new entries to the holders' Telegram channel (needs `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHANGELOG_CHAT_ID`; use `--dry-run` to preview), and `npm run changelog:push:x` tweets them from @trythreews (needs `X_API_KEY`/`X_API_SECRET`/`X_ACCESS_TOKEN`/`X_ACCESS_SECRET`; same `--dry-run` flag). X is the primary holder channel. Skip silently if creds are absent locally.
