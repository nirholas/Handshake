...
- [`PORTAL_WORLD.md`](./PORTAL_WORLD.md): Portal World v1. The two documents behind `/api/portal`: the outline a web page is read into, and the deterministic, seeded world that outline lays out into. Pure layout, so the browser renderer, the GLB exporter, the MCP tools and the npm package all build the same city from the same link.
