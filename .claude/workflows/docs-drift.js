...
const scan = await agent(
  `In the repository root (your current working directory), find every directory that the CLAUDE.md documentation rules require to have a README.md but that has none (or an empty/stub one). Check: every directory under packages/, every directory under workers/, the SDK dirs (sdk/, solana-agent-sdk/, agent-payments-sdk/), and every surface listed in STRUCTURE.md whose mapped directory lacks a README. Read enough of each candidate's code to write a one-line summary of what it actually does. Exclude node_modules, dist, build output, and scratch dirs. Return the full list.`,
