...
//   * Never overclaim, even when the shorter sentence is nicer. This copy used
//     to say "we do not store the names of your rooms or devices" full stop.
//     That was not true: an action log row holds entity ids, and a Home
//     Assistant id is usually a slug of the name it was created with, so
//     "light.sarah_bedroom" IS the name for every practical purpose. The
//     sentence now says what is actually stored and what is not, which is
//     longer and honest. A privacy promise that is 95% true is a false one.
...
export const CONNECT_DISCLOSURE = Object.freeze({
	...
	lines: Object.freeze([
		...
		'We store the address of your home, the name you give it, and a count of how many entities and areas it has. When your agent does something we record which devices it touched, by your Home Assistant\'s own id for them (light.kitchen). We never store their display names, and we never store their states: no record of which lights were on, or when.',
