...
// The API supplies the scale (server-resolved from the same env config that
// builds the 402 accepts), so these tests exercise decimals, not addresses.
...
describe('formatReceiptAmount', () => {
	it('renders 6-decimal stablecoin amounts', () => {
		expect(formatReceiptAmount('10000', USDC_DECIMALS).label).toBe('$0.01');
		expect(formatReceiptAmount('1000000', USDC_DECIMALS).label).toBe('$1.00');
		expect(formatReceiptAmount('2500000', USDC_DECIMALS).label).toBe('$2.50');
	});

	it('keeps precision on sub-cent micropayments instead of showing $0.00', () => {
		expect(formatReceiptAmount('1000', USDC_DECIMALS).label).toBe('$0.001');
		expect(formatReceiptAmount('100', USDC_DECIMALS).label).toBe('$0.0001');
	});

	it('refuses to guess the scale when the server did not supply one', () => {
		// An asset this deployment does not settle in arrives with a null scale.
		expect(formatReceiptAmount('10000', null)).toBe(null);
		expect(formatReceiptAmount('10000', undefined)).toBe(null);
		expect(formatReceiptAmount('10000', 'six')).toBe(null);
		expect(formatReceiptAmount('10000', 99)).toBe(null);
	});

	it('handles missing or malformed amounts', () => {
		expect(formatReceiptAmount(null, USDC_DECIMALS)).toBe(null);
		expect(formatReceiptAmount('', USDC_DECIMALS)).toBe(null);
		expect(formatReceiptAmount('not-a-number', USDC_DECIMALS)).toBe(null);
	});
});
...
describe('totalSpend', () => {
	it('sums priced rows and counts the unpriced ones', () => {
		const s = totalSpend([
			{ amountAtomics: '10000', assetDecimals: USDC_DECIMALS },
			{ amountAtomics: '1000000', assetDecimals: USDC_DECIMALS },
			{ amountAtomics: null, assetDecimals: null },
		]);
