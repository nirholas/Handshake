...
vi.mock('../api/_lib/secret-box.js', () => ({ decryptSecret: async () => 'plaintext' }));
...
describe('snapshot cache', () => {
	...
	it('scans once and serves the snapshot for the whole TTL', async () => {
		const gather = vi.fn(async () => ({ counts: { stranded_funded: 0, stranded_unread: 0 }, sol: {} }));
	...
	it('collapses concurrent reads into one fleet scan', async () => {
		...
		const gather = vi.fn(async () => {
			await gate;
			return { counts: { stranded_funded: 0, stranded_unread: 0 }, sol: {} };
		});
	...
	it('does not cache a failed scan, so a transient outage is retried not frozen', async () => {
		const gather = vi.fn()
			...
			.mockResolvedValueOnce({ counts: { stranded_funded: 0, stranded_unread: 0 }, sol: {} });
