...
describe('withQuickLookBanner', () => {
	it('appends all three banner fields as encoded fragment params', () => {
		const out = withQuickLookBanner('https://three.ws/cdn/a.usdz', {
			title: 'Scout the Fox',
			subtitle: 'Living agent on three.ws',
			callToAction: 'Pin it here for people nearby',
		});
		expect(out).toBe(
			'https://three.ws/cdn/a.usdz' +
	...
	it('skips blank fields and emits only what is present', () => {
		...
		expect(out).toBe('https://three.ws/cdn/a.usdz#callToAction=Meet%20this%20agent');
	});

	it('returns the URL untouched when no usable fields are given', () => {
		expect(withQuickLookBanner('https://three.ws/cdn/a.usdz')).toBe('https://three.ws/cdn/a.usdz');
		expect(withQuickLookBanner('https://three.ws/cdn/a.usdz', {})).toBe('https://three.ws/cdn/a.usdz');
		expect(withQuickLookBanner('https://three.ws/cdn/a.usdz', { title: '' })).toBe('https://three.ws/cdn/a.usdz');
	});

	it('joins onto an existing fragment with & instead of a second #', () => {
		const out = withQuickLookBanner('https://three.ws/cdn/a.usdz#allowsContentScaling=0', { title: 'Scout' });
		expect(out).toBe('https://three.ws/cdn/a.usdz#allowsContentScaling=0&checkoutTitle=Scout');
	});
	...
	it('encodes characters that would break the fragment', () => {
		const out = withQuickLookBanner('https://three.ws/cdn/a.usdz', { title: 'a #1 fox & friend?' });
		expect(out).toBe('https://three.ws/cdn/a.usdz#checkoutTitle=a%20%231%20fox%20%26%20friend%3F');
	});

	it('clamps runaway fields to 80 characters before encoding', () => {
		const long = 'x'.repeat(200);
		const out = withQuickLookBanner('https://three.ws/cdn/a.usdz', { title: long });
		expect(out).toBe(`https://three.ws/cdn/a.usdz#checkoutTitle=${'x'.repeat(80)}`);
	});
