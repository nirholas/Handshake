...
# trimesh has no EXT_meshopt_compression decoder, and a meshopt GLB's buffer 1 is
# a fallback that only exists after decoding, so trimesh dies on the bufferView
# lookup with a bare IndexError. Meshopt is what gltfpack emits and what most of
# three.ws's own avatars ship as, so this was a hard failure on our own assets
# (verified 2026-08-13: /avatars/michelle.glb failed every style in production).
# gltfpack is the same project's CLI and decodes what it encoded: transcode the
# input to plain glTF before trimesh ever sees it.
MESHOPT_EXTENSION = "EXT_meshopt_compression"
GLTFPACK_BIN = os.environ.get("GLTFPACK_BIN", "gltfpack")
GLTFPACK_TIMEOUT_S = 120
...
def _gltf_document(data: bytes, suffix: str) -> Optional[dict]:
...
def _uses_meshopt(document: Optional[dict]) -> bool:
    """True when the asset needs a meshopt decode before any reader can use it.

    Checked against both extension lists: a file that only *uses* the extension
    still stores its geometry in the compressed views, and its fallback buffer
    is empty until something decodes into it.
    """
    if not document:
        return False
...
def _transcode_meshopt(data: bytes, suffix: str) -> bytes:
    ...
    out, so the stylized result is derived from the geometry the author shipped.
    """
...
def _load_single_mesh(data: bytes, suffix: str):
    ...
    if _uses_meshopt(_gltf_document(data, suffix)):
        log.info("input declares %s; decoding with gltfpack before load", MESHOPT_EXTENSION)
        data = _transcode_meshopt(data, suffix)
        suffix = ".glb"
